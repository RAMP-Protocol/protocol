...
- `wire/base.ts` — the wire policy, and the seam every schema routes through.
  ...
  **`parseWire()`**, which is how a message should be parsed. Two things are true of the
  RAMP wire that a schema cannot express, and both live there: an unset message field
  arrives as `null` (proto3 JSON with `EmitUnpopulated`, which is what a RAMP Exchange
  serves, renders it rather than omitting it), and a lowerCamelCase answer is refused at
  every depth rather than parsed into a message with every multiword field missing.
...
`"ext": null` for an unset message field, which the schema alone rejects, and a server that
