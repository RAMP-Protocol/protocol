...
// shape that is silently missing from an example (the R13 class: reflected-offer
// execute examples that omitted expires_at — the exact field the SDK verifier
// fail-closes on).
...
// a missing expiry (ramphelpers.VerifyPresentedOffer: an offer with no
// expires_at is a forever-token and unsafe to honor), so an example without it
// teaches a shape every conformant Exchange rejects. Detection is fence-level
// co-occurrence, the TestDocLicenseTermSemantics idiom: `idempotency_key` marks
// a transaction-request payload, a `signature` key marks an embedded offer.
