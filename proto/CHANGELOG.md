...
One consequence for Go consumers on an older toolchain. `net/url`'s host-colon
strictness is the `urlstrictcolons` GODEBUG, and its default comes from the MAIN
module's `go` directive — so a consumer declaring below 1.26 built `helpers.IsBareHost`
against a parser that accepted `exchange.example::443`, `exchange.example:44:3` and five
near relatives, while the vectors this repo publishes record every one of them refused.
A rule whose answer depends on who compiles it is not the rule, so the second colon is
now refused by the predicate itself. Such a consumer gets refusals it previously did
not; a consumer already on 1.26 sees no change, and no committed vector moves.
