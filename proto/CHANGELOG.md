...
(breaking, pre-1.0).** `LicenseTerm.restrictions`, `.quotas` and `.obligations` carry at
most 64 items each — the bound every other per-message list in this contract already
carries — and `PushResourcesRequest.entries` and `RemoveResourcesRequest.paths` at most
256, the bound a caller-chosen batch carries at `ResourceQuery.uris`. Every committed
...
told its credentials were wrong. Measured: the largest conformant push — 256 entries each
...
stamped when empty; a request that names no bare-host recipient is refused before it is
...
authorisation, tenant binding and per-entry verdicts stay the handler's job.
