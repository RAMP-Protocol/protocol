...
An **unset message field may arrive as `null`**, and the generated Zod schemas now
accept it. proto3 JSON with `EmitUnpopulated` — what `connectserver`'s codec emits, and
therefore what a RAMP Exchange serves — renders every unpopulated message, Struct and map
as `null` rather than omitting it, so `{"ext":null}` is the ordinary shape of a real
response. Every generated schema rejected it, which meant a TypeScript consumer could not
read a conformant answer at all. Pydantic already spelled those fields `X | None`.
