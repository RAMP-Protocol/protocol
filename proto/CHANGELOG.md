...
are fully described by. Six consequences reach anyone re-pinning, and none of them
moves a field, a message or an encoding — `buf breaking` reports nothing.

An **unset message field may arrive as `null`**, and the TypeScript wire policy now
accepts it. proto3 JSON with `EmitUnpopulated` — what `connectserver`'s codec emits, and
therefore what a RAMP Exchange serves — renders an unpopulated **non-optional message
field and a `Struct`** as `null` rather than omitting it, so `{"ext":null}` is the ordinary
shape of a real response. (An unset **map** renders `{}`, and an unset `optional` message
field is omitted outright; only the first two are `null`.) Every generated Zod schema
rejected `null`, which meant a TypeScript consumer could not read a conformant answer at
all. Pydantic already spelled those fields `X | None`.
