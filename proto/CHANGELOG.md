...
mis-addressed request is malformed rather than a domain-level failure, and that same check
is what stops a request captured at one recipient from being replayed at another.

The signature does not already establish this, in either case that matters. On a relayed
path each hop signs its own `@target-uri`, so the agent's signature covers the Broker's URL
and the Exchange never sees an agent statement about where the request was meant to go. On
a direct hop the signature does cover the recipient's URL, but that URL came from a fetched,
cached manifest — so it proves the sender signed *the URL it dialled*, not that the URL was
right. The body field is covered by the sender's content-digest, which a relaying
intermediary cannot alter without breaking the inner signature. Two messages are exempt and
