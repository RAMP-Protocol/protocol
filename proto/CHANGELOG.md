...
`ext`/`ext_critical`; the Exchange no longer promotes it from `ext` at discovery. The field
is `optional` — when omitted the Exchange defaults to `STATIC`. Offer-side
`ResourceIdentity.resource_mutability` is unchanged.
