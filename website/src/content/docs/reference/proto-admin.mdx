...
The server-side lifecycle record of the transaction's reporting obligation: the `ObligationState`, the reported consumed quantity (mirroring the wire's `Usage.consumed_quantity` `int32`, absent until a report has been accepted), the report deadline, when a report arrived, and when the obligation was minted. Named apart from `ramp.v1.ReportingObligation`, the agent-facing requirements contract this state was minted from.
