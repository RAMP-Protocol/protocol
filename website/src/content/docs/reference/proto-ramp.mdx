...
| `license_id` | optional string | 5 | Commercial license or subscription identifier (same role as CoMP `aisysuse.lid`) |
...
| 6 | RESTRICTION_NOT_SATISFIED | Accepted term's restriction not satisfied by the request (axes in `restriction_mismatches`, same `RestrictionKind` vocabulary as the terms) |
| 7 | REPORTING_OVERDUE | Buyer has overdue reports |
| 8 | OFFER_EXPIRED | Offer TTL exceeded |
| 9 | SIGNATURE_INVALID | Signature verification failed |
| 10 | QUOTA_EXCEEDED | Subscription access count exhausted for this period |
| 11 | DELEGATION_EXPIRED | Biscuit delegation token expired |
| 12 | SCOPE_INSUFFICIENT | Requester scopes don't cover this resource |
