...
| Field | Type | Number | Description |
|---|---|---|---|
| `verifier` | string | 1 | Domain of the attesting party. Keys in the verifier's WBA directory (the JWK Set at `https://{verifier}/.well-known/http-message-signatures-directory`; `role=ROLE_PUBLISHER` when self-attesting, `ROLE_EXCHANGE` for a standalone vendor) |
| `keyid` | string | 2 | RFC 7638 thumbprint of the verifier's signing key (the RFC 9421 `keyid`) |
...
| `revoked` | repeated string | 2 | Complete list of revoked key thumbprints (RFC 7638) at `as_of` |
