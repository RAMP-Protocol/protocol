...
| `token` | bytes | 6 | Token bytes — JWT (base64url) or Biscuit (binary, base64) |
| `token_format` | string | 7 | Token format: "jwt" (default) or "biscuit-v2" |
...
| `issuer` | optional string | 11 | Token issuer — OIDC issuer URL or GNAP grant server (for JWT/GNAP validation) |
| `ext` | Struct | 15 | Extension point |
...
| `signature` | string | 9 | **REQUIRED.** JWS (EdDSA) over the canonical Offer (incl. `terms`, `pricing`, `expires_at`) |
