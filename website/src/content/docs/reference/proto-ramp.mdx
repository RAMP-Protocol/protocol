...
- `restrictions`, `quotas` and `obligations` each carry at most 64 items, the bound every other per-message list in the contract carries. Only one restriction per axis is valid anyway, so the cap sits far above any conformant term; it bounds what one term can carry, not the cost of checking it.
