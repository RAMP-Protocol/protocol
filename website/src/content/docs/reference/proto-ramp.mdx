...
`data_schema` carries a normative safety rule set, because a client reads it out of a **third party's** manifest and validates against it before any signature has been checked: same-document references only, one pinned dialect, a 16KB size cap, a 32-container depth cap, a restricted `pattern` alphabet, and `format` left as an annotation. The bounds are numbers in the contract rather than each implementation's choice, because a schema is validated at both ends of one registration and a limit invented by one side refuses payloads the other accepts. See [Registration schema rules](/protocol/exchange-manifest/#registration-schema-rules) for the list and the reasoning.
