...
Agent sends to Broker (Step 1), carried by `BrokerService.Resolve`. Carries an `idempotency_key` (field 10, `min_len:1`/`max_len:255`, deduped per verified RFC 9421 signer) so a retried Resolve returns the original result rather than re-charging.
