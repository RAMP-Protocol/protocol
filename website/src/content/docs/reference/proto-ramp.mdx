...
Scoped, time-limited, spend-capped credential. The credential itself is opaque bytes carried in `token`, and `token_format` selects how to interpret them: `"biscuit-v3"` (default) or `"jwt"`. The format is never encoded as a prefix inside the token bytes. Biscuit v3 is the default and the only format with a fully specified verification path in v1; [Biscuit authorization tokens](https://biscuitsec.org) are Ed25519-signed block chains that can be attenuated (holder adds restrictions) without going back to the issuer, each block narrowing permissions -- never widening them.
...
**Delegation claims.** The claim/fact vocabulary carried inside the token is **not re-listed here** — the authoritative registry is the [Delegation-Claims Profile](/protocol/authentication/#delegation-claims-profile) table in `authentication.mdx`. The one claim worth restating: every delegation **MUST** carry a **subject/holder binding** (Biscuit: the holder key the token is sealed/attenuated to; JWT: `sub` + `cnf` proof-of-possession key) — the request-signing key MUST equal the bound key, or the token is rejected. Without it the token is bearer-usable. Everything else (scope, spend cap, expiry, etc.) is optional and defined in that single source of truth.
