...
| 7 | QUOTA_EXCEEDED | Contributor push quota exceeded |
...
|---|---|---|
| 0 | UNSPECIFIED | |
| 1 | URL_EXPIRED | Signed URL expired |
| 2 | URL_SIGNATURE_MISSING | Signed-URL signature missing |
| 3 | URL_EXPIRY_MISSING | Signed-URL expiry missing |
| 4 | URL_SIGNATURE_MISMATCH | Signed-URL signature mismatch |
| 5 | AGENT_KEY_MISSING | Fetcher did not present its public key |
| 6 | PROOF_SIGNATURE_MISSING | Proof-of-possession signature missing |
| 7 | KEYID_MISMATCH | PoP keyid mismatch |
| 8 | THUMBPRINT_MISMATCH | Presented key's thumbprint ≠ `agent_identity_hash` |
| 9 | PROOF_CREATED_MISSING | PoP `created` parameter missing |
| 10 | PROOF_EXPIRY_MISSING | PoP expiry missing |
| 11 | PROOF_EXPIRED | PoP expired |
| 12 | PROOF_SIGNATURE_INVALID | PoP signature invalid |
...
<EnumTable enum="PricingModel" numbers />
...
| 5 | RESTRICTION_NOT_SATISFIED | Accepted term's restriction not satisfied by the request (axes in `restriction_mismatches` — on `ErrorDetail.transaction_denial` for a single denial, or on `TransactionResultItem` for a batch item; same `RestrictionKind` vocabulary as the terms) |
| 6 | REPORTING_OVERDUE | Buyer has >20% overdue reports (MAY threshold) |
...
| 10 | DELEGATION_INVALID | Delegation missing, unverifiable (broken chain linkage or holder-binding mismatch), expired, or its attenuation does not check out |
| 11 | SCOPE_INSUFFICIENT | Requester scopes don't cover this resource |
| 12 | ENTITLEMENT_MISSING | No entitlement token presented for a subscription-only offer |
...
| 15 | ENTITLEMENT_WRONG_BUYER | Token's `subscriber_org` does not match the asserted buyer |
...
| 18 | ENTITLEMENT_STALE_ATTENUATION | Token authority sound but the per-request attenuation is missing/stale |
