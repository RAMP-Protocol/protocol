...
Agent asks the Exchange to create its account. The caller's identity is proven by the verified request signature, never asserted in the body; the operator-defined business payload rides in `registration_data` and is passed through to the Exchange's system of record uninspected.
...
Validation rules (wire-enforced via protovalidate):
- `resource_mutability` rejects `RESOURCE_MUTABILITY_UNSPECIFIED` (the field's `enum.not_in:[0]` rule). It is `optional`, so an omitted value is accepted — the Exchange defaults the Offer to `STATIC` at build time.
