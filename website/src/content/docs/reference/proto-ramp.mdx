...
| 12 | ENTITLEMENT_MISSING | No entitlement biscuit presented for a subscription-only offer |
| 13 | ENTITLEMENT_MALFORMED | Entitlement biscuit failed Biscuit v2 decode |
| 14 | ENTITLEMENT_EXPIRED | Entitlement biscuit's validity window has passed |
| 15 | ENTITLEMENT_WRONG_BUYER | Biscuit's `subscriber_org` does not match the asserted buyer |
...
| 18 | ENTITLEMENT_STALE_ATTENUATION | Biscuit authority sound but the per-request attenuation is missing/stale |
