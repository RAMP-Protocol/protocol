...
Source: [`proto/ramp/v1/ramp.proto`](https://github.com/postindustria-tech/ramp-protocol/blob/main/proto/ramp/v1/ramp.proto)
...
| `domain` | string | 2 | Requester's domain -- used for public key lookup at `{domain}/.well-known/ramp-agent.json` |
...
| Field | Type | Number | Description |
|---|---|---|---|
| `verifier` | string | 1 | Domain of the attesting party. Keys at `https://{verifier}/.well-known/ramp-verifier.json` |
| `kid` | string | 2 | Key ID from the verifier's JWKS |
...
| `package` | optional comp.v1.Package | 5 | CoMP Package with signed URL |
...
| `expires_at` | optional Timestamp | 9 | Signed URL expiry |
| `agent_identity_hash` | string | 10 | **REQUIRED.** Agent identity bound into URLs |
...
| `package` | comp.v1.Package | 4 | CoMP Package with signed URL populated |
...
| `expires_at` | optional Timestamp | 8 | When the signed URL expires |
...
| `exchanges` | repeated AuthorizedExchange | 4 | Authorized Exchanges |
...
Served at `/.well-known/ramp-exchange.json`. Machine-readable self-description of a RAMP Exchange. See [Exchange Manifest](/protocol/exchange-manifest/) for details.
...
| `package` | comp.v1.Package | 7 | CoMP Package with retrieval.endpoint populated |
...
| `expires_at` | optional Timestamp | 11 | When the delivery endpoint expires |
...
|---|---|---|
| 0 | UNSPECIFIED | |
| 1 | MARKETPLACE | Direct query or Exchange discovery |
