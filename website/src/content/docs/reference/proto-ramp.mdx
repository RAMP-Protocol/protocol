...
- `REFERENCE_ONLY` requires `license.uri` to be non-empty; a `License` with a `uri` requires a `uri_digest`.
- At most one `Restriction` per `kind`; a token cannot be both permitted and prohibited.
