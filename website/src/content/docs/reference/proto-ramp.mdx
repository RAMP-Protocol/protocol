...
Request correlation is **not** carried in the proto body. It rides on the `X-Request-ID` HTTP header, and cross-system tracing uses W3C Trace Context (`traceparent` / `tracestate`). The former `request_id` fields have been removed from every `ramp.v1` message. One admin-plane carve-out exists by the persisted-identifier rule: the forensic evidence read (`ramp.admin.v1.RequestCorrelation`) states, after the fact, the correlation id the Exchange *persisted* for a transaction — no live request or response carries one in its body.
:::
