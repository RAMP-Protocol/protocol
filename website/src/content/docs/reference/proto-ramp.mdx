...
| `license_duration_months` | optional int32 | 7 | License duration in months (field 6 retired — was `revshare`; settlement is off-protocol) |
...
| `scopes` | repeated string | 7 | Biscuit scope-gating (AND-semantics). Empty = public; hierarchical (`dist:*` covers `dist:US`). |
...
Retired (pre-v1, renumbered cleanly): the old per-unit variants (`PER_ACCESS` / `PER_TOKEN` / `PER_FETCH` / `PER_PAGE` / `PER_MINUTE` / `PER_RECORD` / `TRAINING`) collapse into `PER_UNIT` + `unit`; `SUBSCRIPTION` is `FREE` + `scopes`; `ATTRIBUTION` / `CONTRIBUTION` are `Obligation.kind`s; `REVENUE_SHARE` is removed (settlement is off-protocol).
...
|---|---|---|
| 0 | UNSPECIFIED | Unset — rejected at ingest |
| 1 | ENUMERATED | Machine fields are authoritative. Pricing MUST be present. |
| 2 | REFERENCE_ONLY | Only `License.uri` is authoritative (MUST be non-empty). Machine `restrictions`/`quotas`/`obligations` MUST be absent — the agent reads the document. Pricing is still required. |
