...
Identifies who is making a request, what scopes they have, and an optional delegation chain. Used in `ResourceQuery` and `RAMPRequest`.
...
| `delegation` | optional Delegation | 9 | Present when requester acts on behalf of another entity |
| `ext` | Struct | 15 | Extension point |
...
One hop in the request forwarding chain (Agent -> Broker -> Exchange). Each hop authenticates by adding its own RFC 9421 HTTP Message Signature, proving the request passed through legitimately. Modeled after OpenRTB schain (Supply Chain Object). Responses do not retrace the chain — the terminal Exchange returns directly to the originating agent.
...
| `signature_label` | optional string | 4 | Binds this hop to its RFC 9421 `Signature` header label, so a verifier matches a hop to its signature without inferring by ordinal position or domain |
...
| `terms` | repeated LicenseTerm | 19 | Licensing terms for this offer (supersedes `restrictions` when present). See [Universal Licensing Core](#universal-licensing-core). |
| `ext` | Struct | 15 | Extension point |
...
The Universal Licensing Core replaces the flat `AccessRestrictions` structure with a structured, cross-domain-portable licensing model. A resource carries zero or more `LicenseTerm` entries — each term is a complete commercial arrangement. Multiple terms are the normal case: a news article may be free for academic use and paid for commercial use; a stock photo may be perpetually licensed with an impressions cap.
...
- Unknown tokens in `restrictions[].permitted` / `prohibited` produce `PushResourcesResponse.warnings[]` but do NOT cause rejection (forward-compatible).

| Field | Type | Number | Description |
|---|---|---|---|
| `semantics` | TermSemantics | 1 | How the Exchange interprets the machine fields |
| `license` | optional License | 2 | Governing license document. Authoritative for REFERENCE_ONLY terms. |
...
| `pricing` | optional Pricing | 6 | Commercial terms. REQUIRED for ENUMERATED; absent for REFERENCE_ONLY. |
...
| Field | Type | Number | Description |
|---|---|---|---|
| `uri` | optional string | 1 | Authoritative license URI (e.g. `"https://creativecommons.org/licenses/by/4.0/"`) |
| `title` | optional string | 2 | Human-readable license name (e.g. `"CC BY 4.0"`) |
| `spdx_expression` | optional string | 3 | SPDX expression (e.g. `"CC-BY-4.0"`, `"MIT"`) |

### Restriction

A single constraint on one licensing dimension: function (what), geography (where), or user-type (who).

**Evaluation rule**: an agent is in-scope when it matches at least one `permitted[]` token AND none of the `prohibited[]` tokens. Empty `permitted[]` = any value permitted on this axis.

**Vocabulary sources**: `vocab/restriction-values/function.json`, `vocab/restriction-values/geography.json`, `vocab/restriction-values/user-type.json`.
...
| `critical` | bool | 4 | When true, an agent that cannot verify compliance MUST decline this term. |
...
**Metric vocabulary**: `vocab/quota-metrics.json`. Standard metrics: `accesses`, `tokens`, `input-tokens`, `display-words`, `impressions`, `copies`, `seats`, `units-manufactured`.

| Field | Type | Number | Description |
|---|---|---|---|
| `metric` | string | 1 | Unit being capped. Must match a metric in `vocab/quota-metrics.json`. |
| `limit` | int64 | 2 | Maximum allowed value in the given window. |
| `window` | QuotaWindow | 3 | Time window over which the limit accumulates. |

### Obligation

A post-use behavioral requirement attached to a `LicenseTerm`. Replaces the retired `PRICING_MODEL_ATTRIBUTION` and `PRICING_MODEL_CONTRIBUTION` pricing model values.

| Field | Type | Number | Description |
|---|---|---|---|
| `kind` | ObligationKind | 1 | What the agent must do. |
| `trigger` | ObligationTrigger | 2 | When the obligation activates. |
| `description` | optional string | 3 | Human-readable description. Required for `OBLIGATION_KIND_OTHER`. |
...
| 5 | FREE | No charge |
| ~~6~~ | ~~ATTRIBUTION~~ | **Retired in v1.1.** Use `Obligation.kind = ATTRIBUTION` in a `LicenseTerm` instead. |
| ~~7~~ | ~~CONTRIBUTION~~ | **Retired in v1.1.** Use `Obligation.kind = CONTRIBUTION` in a `LicenseTerm` instead. |
...
| Value | Name | Description |
|---|---|---|
| 0 | ENUMERATED | Machine fields are authoritative and enforced by the Exchange. Pricing MUST be present. |
| 1 | REFERENCE_ONLY | Only `License.uri` is authoritative. Machine fields are informational; the Exchange MUST NOT auto-enforce them. Agent must read the license URI before using. |
...
| Value | Name | Description |
|---|---|---|
| 0 | FUNCTION | What the agent may do. Tokens from `vocab/restriction-values/function.json`. |
| 1 | GEOGRAPHY | Where the agent operates. ISO 3166-1 alpha-2 + `EU`, `EEA`, `*`. |
| 2 | USER_TYPE | Who the agent is. Tokens from `vocab/restriction-values/user-type.json`. |
| 3 | OTHER | Custom dimension; description in `Restriction.description`. |
...
| Value | Name | Description |
|---|---|---|
| 0 | HOURLY | Resets each hour |
| 1 | DAILY | Resets each day |
| 2 | MONTHLY | Resets each month |
| 3 | TOTAL | Lifetime cap — never resets |

### ObligationKind

What the agent must do after use. `ATTRIBUTION` and `CONTRIBUTION` replace the retired `PRICING_MODEL_ATTRIBUTION` / `PRICING_MODEL_CONTRIBUTION`.

| Value | Name | Description |
|---|---|---|
| 0 | ATTRIBUTION | Credit the author or publisher |
| 1 | CONTRIBUTION | Good-faith payment (amount suggested, not contractually fixed) |
| 2 | SHARE_ALIKE | Derivatives must use the same license (CC-BY-SA / GPL style) |
| 3 | NETWORK_COPYLEFT | Network service triggers copyleft (AGPL style) |
| 4 | NOTICE | Include the specified copyright notice |
| 5 | OTHER | Described in `Obligation.description` |
...
| Value | Name | Description |
|---|---|---|
| 0 | ON_USE | Triggered on any use of the resource |
| 1 | ON_DISTRIBUTION | Triggered when copies are distributed |
| 2 | ON_NETWORK_SERVICE | Triggered when served over a network (AGPL style) |
| 3 | ON_DERIVATIVE | Triggered when a derivative work is produced |
