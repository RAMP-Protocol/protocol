...
An agent's manifest is much smaller. It is a role marker, plus the optional
`identity_documents` block naming where the agent serves the documents that
...
Both members are URI references — relative, as above, or absolute. A consumer
resolves each against the URL it fetched `ramp.json` from, and the result must be
on the **exact same origin** as that URL: `https`, the same hostname, the same
effective port, and no userinfo. A subdomain is refused, which is stricter than
the rule for `endpoint`; [Identity documents](/protocol/exchange-manifest/#identity-documents)
explains why.

Omitting `wba_directory` keeps today's behaviour — a consumer falls back to the
canonical path below on the host that served the manifest. Omitting
`signature_agent_card` means the card is **not advertised**; RAMP defines no
canonical path for it, so nothing may guess one.
