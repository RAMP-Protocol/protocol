...
An agent's manifest is much smaller: a role marker. Its signing keys are not here — they live in the WBA directory described below, which a verifier resolves from the covered `Signature-Agent` header.
