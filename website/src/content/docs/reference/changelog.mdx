...
- **`TermSemantics`** — `ENUMERATED` (machine fields authoritative; enforced downstream at reconciliation; Pricing required) vs `REFERENCE_ONLY` (only `License.uri` is authoritative; machine restrictions/quotas/obligations absent; Pricing still required).
