...
now documents what it always did but never said: verification fails closed, the
signature is checked before the expiry, agent binding is enforced by default, and
the delivery endpoint holds public keys only.
