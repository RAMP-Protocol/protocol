...
- **`Quota`** — usage cap that gates term validity, not billing. Metrics from `vocab/quota-metrics.json`: `accesses`, `tokens`, `display-words`, `impressions`, `units-manufactured`, and more.
- **`Obligation`** — post-use behavioral requirement. Replaces the retired `PRICING_MODEL_ATTRIBUTION` / `PRICING_MODEL_CONTRIBUTION` (attribution and contribution are obligations, not payment models). Kinds: `ATTRIBUTION`, `CONTRIBUTION`, `SHARE_ALIKE`, `NETWORK_COPYLEFT`, `NOTICE`, `OTHER`.
- **`TermSemantics`** — `ENUMERATED` (machine fields authoritative; Exchange enforces; Pricing required) vs `REFERENCE_ONLY` (only `License.uri` is authoritative; Exchange passes through unchanged).
...
- `Offer.terms` (field 19) — repeated LicenseTerm from the publisher catalog. Agents SHOULD prefer this over the legacy `Offer.restrictions` when present.
