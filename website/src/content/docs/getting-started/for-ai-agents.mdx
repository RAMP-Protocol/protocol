...
| `signature` | Ed25519 signature over `(id, domain, uris, scopes)` | (computed by SDK) |

The SDK signs every request with your Ed25519 private key. The Exchange verifies it against the public key published at `{domain}/.well-known/ramp.json` (`role=ROLE_AGENT`), selected by `kid` from the manifest's `public_keys`.
