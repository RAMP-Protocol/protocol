...
- **Go**: Import generated types from [`gen/go/`](https://github.com/RAMP-Protocol/protocol/tree/main/gen/go/) and the Connect-Go client from `gen/go/ramp/v1/rampv1connect`.
- **TypeScript**: Validate messages with the generated Zod schemas from [`gen/ts/`](https://github.com/RAMP-Protocol/protocol/tree/main/gen/ts/). The npm package is not yet published.
- **Python**: Parse messages with the generated Pydantic models from [`gen/python/`](https://github.com/RAMP-Protocol/protocol/tree/main/gen/python/). The PyPI package is not yet published.
</Aside>

Configure it with your agent identity, signing key, and budget:

```go
client, _ := ramp.NewClient(ramp.Config{
    AgentID:     "acme-research-bot",
    AgentDomain: "agents.acme.com",
...
At Level 2, an independent verification vendor (whose keys are published in its WBA directory at `https://{verifier}/.well-known/http-message-signatures-directory`) has crawled and measured the content. Your agent trusts the vendor's attestation without re-verifying.
...
1. Register your agent with a participating Exchange — `Register` in Go, `register` in TypeScript and Python. The Exchange creates your account and bills it based on your verified request signature, never on anything you send. Address the call by naming the Exchange's bare domain on the request: the SDK resolves the endpoint from that Exchange's own manifest, reads the terms revision it publishes from a freshly fetched copy, and echoes it so the signature records what you accepted.
2. Check the account with `GetAccountStatus` / `getAccountStatus` before you transact. An empty account handle means you hold no account there yet; an account that exists but is not active is waiting for the operator to switch it on, and the remedy is to wait or contact them rather than to register again. An account is per-Exchange, so a client can hold several — the same client registers at each.
3. Install the SDK, declare your `supported_profiles`, and configure your budget
4. Replace your per-provider integrations with `client.Fetch()`
