...
**Scopes** filter what the Requester can access. Examples: `subscription:marketdata-2026`, `credit:read`, `academic:*`, `*` (unrestricted). The Exchange returns only resources matching the declared scopes.

**Delegation with JWTs** enables offline-verifiable, narrowable authorization. A principal (organization, IAM, upstream agent) issues a delegation JWT. Each holder in the chain can narrow permissions by issuing a child delegation JWT — restrict scopes, lower spend caps, shorten expiry — but can never widen them. Zero round trips to the original issuer, regardless of how many delegation levels deep.
...
For the full identity model — key lookup, child delegation JWT examples, scope-based catalog filtering, and denial reasons — see [Authentication](/protocol/authentication/).
