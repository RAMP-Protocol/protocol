...
If you self-manage, deploy using the [Terraform module](https://github.com/RAMP-Protocol/protocol/tree/main/terraform/aws-ramp-edge/):
...
| Today | With RAMP |
|---|---|
| Build custom API integrations per consumer | One protocol handles all AI requesters |
...
| Each AI company wants a separate deal | One Exchange relationship handles all requesters |
...
You **may** declare `resource_mutability` via the typed `ResourceEntry.resource_mutability` field; if you omit it, the Exchange sets `STATIC` when it builds the Offer (it reads the typed field, not `ext`). For DYNAMIC resources, the Exchange also sets `data_as_of` — a timestamp indicating when the data was current. Agents can filter by freshness using `max_data_age` (e.g., "credit report updated within 7 days").
...
3. The attestation is pushed to the Exchange via `CatalogService.PushResources` — the SDK's catalog client in Go, TypeScript or Python signs and sends it; run `ValidateResourceEntry` (or its Python/TypeScript twin) on each entry first, which the SDK also ships (see [Publisher Onboarding](/getting-started/publisher-onboarding/#push-your-catalog-with-the-sdk))
...
**Key publication**: Your domain hosts a WBA directory — a JWK Set at `/.well-known/http-message-signatures-directory` — carrying your public keys (inline RFC 7517 Ed25519 JWKs, each identified by its RFC 7638 thumbprint). Agents and Exchanges fetch this to verify your attestation signatures; keys no longer live in `ramp.json`. RAMP has no dedicated verifier role; a verifier publishes under its operator's role — here `ROLE_PUBLISHER` for self-attestation.
...
Your public verification keys are published separately in your WBA directory (the JWK Set at `/.well-known/http-message-signatures-directory`), not in `ramp.json`.
...
No. Your Exchange handles all requester relationships. AI companies buy from the Exchange, not from you directly.
