...
2. Fetch the requester's public key from `{domain}/.well-known/ramp-agent.json`
...
2. Verify the authority block (block 0) is signed by the Ed25519 key published at `{principal_domain}/.well-known/ramp-agent.json`
...
1. Fetch the intermediary's public key from `{hop.domain}/.well-known/ramp-agent.json`
...
1. Fetches the signer's public key from `https://{verifier}/.well-known/ramp-verifier.json`
...
**Exchange Manifest** -- Your Exchange serves a machine-readable self-description at `/.well-known/ramp-exchange.json`. This tells Brokers and agents what your Exchange supports (pricing models, delivery methods, accepted verifiers, endpoints) without trial-and-error. The manifest includes:
