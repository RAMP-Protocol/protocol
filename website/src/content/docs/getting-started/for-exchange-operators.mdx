...
3. Verify the RFC 9421 HTTP Message Signature (alg=EdDSA) on the request — it covers the HTTP request components (`@method`, `@target-uri`, `content-digest`, and the signature parameters), so the request body (the serialized `Requester`/query) is bound via `content-digest`. There is no in-message body-tuple signature.
...
4. **Verify holder binding** — the key that signed the request (RFC 9421) MUST equal the key the token is sealed/attenuated to; reject with `DENIAL_REASON_DELEGATION_INVALID` if they differ. This is mandatory: it is what makes a stolen token unusable, so a token possessed without its signing key grants nothing.
5. Evaluate all check conditions across all blocks — scope restrictions, spend caps, URI constraints
...
| `DENIAL_REASON_DELEGATION_INVALID` | Delegation token missing, unverifiable (bad signature chain or holder-binding mismatch), expired (`expires_at` in the past), or its attenuation does not check out |
