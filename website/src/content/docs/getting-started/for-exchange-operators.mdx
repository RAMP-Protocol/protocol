...
5. Check `expires_at` against current time — reject expired tokens with `DENIAL_REASON_DELEGATION_INVALID`
...
| `DENIAL_REASON_DELEGATION_INVALID` | Biscuit delegation token `expires_at` is in the past |
