...
2. **ExecuteTransaction** -- AI agent says "I'll take it." Your Exchange checks the requester's balance, logs the transaction, and returns a signed URL.
3. **ReportUsage** -- AI agent reports what it did with the content. Mandatory. You track compliance.

That's the whole protocol. Three RPCs. Your billing adapter plugs in where `Authorize(requester, amount)` and `Record(transaction)` are called.
...
ads.txt                  →  ramp.json (authorized providers)
...
6. Done. Their content is now available to AI requesters through your Exchange
...
2. Fetch the requester's public key from their WBA directory (the JWK Set at `{domain}/.well-known/http-message-signatures-directory`, `role=ROLE_AGENT`), identified by its RFC 7638 thumbprint (the RFC 9421 `keyid`)
...
2. Verify the authority JWT is signed by the Ed25519 key published in the principal's WBA directory (the JWK Set at `{principal_domain}/.well-known/http-message-signatures-directory`, `role=ROLE_AGENT`)
...
5. Evaluate the grant's registered claims — scope restrictions, spend caps (`max_spend_cents`), access caps (`max_accesses`) — and confirm they cover the request (URI access is gated through scopes). For the JWT this means checking the registered claims along the `cnf` chain.
...
1. For each signature, fetch the signer's public key from their WBA directory (the JWK Set at `{signer-domain}/.well-known/http-message-signatures-directory`, `role=ROLE_AGENT`), identified by its RFC 7638 thumbprint (the RFC 9421 `keyid`)
...
When the requester holds a subscription (identifiable via the requester's `delegation` and/or subscription scopes), populate `SubscriptionQuotaInfo` entries on offers and transaction responses. This proactive quota signaling lets agents track remaining allowance and avoid committing to transactions they cannot complete. Track `max_accesses` and `quota_period` from the `Delegation` message when present.

### New Denial Reasons to Implement

RAMP defines identity- and entitlement-related denial reasons your Exchange MUST return when appropriate. The delegation/scope reasons:
...
And the entitlement/subscription family, for subscription-gated offers:

| Denial Reason | When to Return |
|---|---|
| `DENIAL_REASON_ENTITLEMENT_MISSING` | No entitlement token presented for a subscription-only offer |
| `DENIAL_REASON_ENTITLEMENT_MALFORMED` | Entitlement token failed to decode |
| `DENIAL_REASON_ENTITLEMENT_EXPIRED` | Entitlement token's validity window has passed |
| `DENIAL_REASON_ENTITLEMENT_WRONG_BUYER` | Token's `subscriber_org` does not match the asserted requester |
...
This is the identity/entitlement subset with operator guidance; the complete, authoritative `DenialReason` enum is rendered from the proto on the [proto reference](/reference/proto-ramp/). For the full identity model specification — Requester fields, the JWT chain, scope format, and key management — see [Authentication](/protocol/authentication/).

## Attestation Verification

Your Exchange verifies `ResourceAttestation` signatures at catalog push time. When a provider or verification vendor pushes resource metadata via `CatalogService.PushResources`, your Exchange:

1. Fetches the signer's public key from their WBA directory (the JWK Set at `https://{verifier}/.well-known/http-message-signatures-directory`), identified by its RFC 7638 thumbprint (the RFC 9421 `keyid`)
...
**Exchange Manifest** -- Your Exchange serves a machine-readable self-description at `/.well-known/ramp.json` (`role=ROLE_EXCHANGE`). This tells Brokers and agents what your Exchange supports (pricing models, delivery methods, accepted verifiers, endpoints) without trial-and-error. Your offer-signing keys are published separately in your WBA directory (the JWK Set at `/.well-known/http-message-signatures-directory`), not in `ramp.json`. The manifest includes:
...
- Subscription deals give your providers an advantage (zero marginal cost for the requester)
...
type BillingAdapter interface {
    Authorize(ctx, billingRef, amount) → (ok, error)
    Record(ctx, transaction) → error
    CheckSubscription(ctx, billingRef, providerGroup) → (sub, error)
}
// billingRef is the account handle the Exchange minted at Register and
// resolves from the verified request signature on every call.
```
