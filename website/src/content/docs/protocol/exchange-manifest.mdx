...
| `endpoint` | string | No | ExchangeService endpoint URL. **Must be on the host that serves this manifest, or a subdomain of it, and must not carry userinfo** — see [Endpoint host binding](#endpoint-host-binding) |
...
`endpoint` **MUST** be on the same host that serves this manifest, or on a
subdomain of it, and **MUST NOT** carry userinfo. An Exchange at
`exchange.example` may advertise `https://exchange.example/v1` or
`https://api.exchange.example/v1`; it may not advertise `https://cdn.other.example/v1`.
...
message WellKnownManifest {
  ...
  optional string endpoint = 12;                   // ExchangeService URL (serving host, or a subdomain)
