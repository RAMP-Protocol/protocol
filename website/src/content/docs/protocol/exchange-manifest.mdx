...
| `endpoint` | string | No | ExchangeService endpoint URL. **Must be on `domain` or a subdomain of it, and must not carry userinfo** — see [Endpoint host binding](#endpoint-host-binding) |
...
message WellKnownManifest {
  ...
  optional string endpoint = 12;                   // ExchangeService URL (same host or a subdomain)
