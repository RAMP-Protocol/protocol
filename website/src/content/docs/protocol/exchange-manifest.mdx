...
**Endpoint**: `https://{domain}/.well-known/ramp.json` — the single canonical file every RAMP participant serves, differentiated by `role`. The legacy per-role `/.well-known/ramp-exchange.json` is **removed in v1.1.0**.
...
|---|---|---|---|
| `ver` | string | Yes | RAMP protocol version (`"1.1"`) |
...
RAMP defines **one** well-known file. Every participant serves a `WellKnownManifest` at `/.well-known/ramp.json`; the `role` field says which participant it is. (v1.0 used four per-role files — `ramp-agent.json`, `ramp-exchange.json`, `ramp-verifier.json`, and the provider `ramp.json`; the per-role variants are removed in v1.1.0.)
...
| Does this Exchange speak protocol v1.1? | `protocol_versions_supported` |
...
The Exchange manifest is a `WellKnownManifest` with `role=ROLE_EXCHANGE`, defined in [`ramp.proto`](/reference/proto-ramp/). The legacy `ExchangeManifest` message is **deprecated** (kept on the wire one cycle); its `keys_uri` pointer is replaced by inline `public_keys`. Exchange-relevant fields:
