...
- The match is on a **full dot-delimited label boundary**. `evil-a.com` is not a
  subdomain of `a.com`; a bare suffix comparison gets that wrong, and it is the
  mistake an attacker registers a domain to exploit.
- The **port is not compared**. TLS binds hostnames, not ports, so
  `https://exchange.example:8443/v1` is a valid endpoint for `exchange.example`.
