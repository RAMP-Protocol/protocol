...
2. Each forwarding hop's labeled RFC 9421 HTTP Message Signature -- for each one, fetch public key from that hop's WBA directory at `{hop.domain}/.well-known/http-message-signatures-directory`, Ed25519 verify (each hop signs the request plus the prior hop's signature)
...
2b. Verify forwarding signature chain (if any forwarding hops added labeled signatures):
   - For each hop's labeled RFC 9421 signature: fetch public key from that
     hop's WBA directory at {hop.domain}/.well-known/http-message-signatures-directory
