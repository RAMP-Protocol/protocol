...
### Broker Fans Out with Intermediary Chain
...
1. `requester.signature` -- fetch public key from `claude.ai/.well-known/ramp.json`, Ed25519 verify
2. `intermediaries` chain -- for each hop, fetch public key from `{hop.domain}/.well-known/ramp.json`, Ed25519 verify
...
The Broker preserves the agent's original `requester` message (including the agent's signature) and appends an `IntermediaryHop` to the `intermediaries` array. The Exchange verifies both: the agent's identity (via `requester.signature` + public key fetched from `requester.domain/.well-known/ramp.json`) and each intermediary in the chain (via each hop's `signature` + public key fetched from `{hop.domain}/.well-known/ramp.json`).
...
2b. Verify intermediaries chain (if intermediaries array is non-empty):
   - For each IntermediaryHop: fetch public key from {hop.domain}/.well-known/ramp.json
