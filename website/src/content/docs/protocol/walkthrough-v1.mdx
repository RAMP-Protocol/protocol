...
The Exchange looks up the article in its catalog and returns an offer. It does not filter the offer against the agent's `intended_use` — the restrictions ride on the offer's terms, and the agent self-selects the term whose `RESTRICTION_KIND_FUNCTION` permitted list covers its intended use (`ai-input`); enforcement happens downstream at accept → report → reconcile:
