...
`PushResources` is atomic: if any entry fails a hard validation rule — the envelope rules above, a term's shape rule, or a bare unregistered unit or metric — the **whole submission is rejected** (`InvalidArgument`, naming the offending entry) and **nothing is persisted**. The publisher fixes and resubmits the full set. Membership / lint issues are returned in `PushResourcesResponse.warnings[]` and do not block.
