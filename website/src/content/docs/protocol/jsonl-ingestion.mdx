...
| `resource_mutability` | string (enum) | no | `ResourceEntry.resource_mutability` (`ResourceMutability`) | `STATIC`/`DYNAMIC`/`LIVE`; drives delivery-time hash-verification behavior |
...
`resource_mutability` is a typed `ResourceEntry` field carried straight through to `Offer.identity.resource_mutability`. `previews[]` still has no typed field on `ResourceEntry` — it rides inside `ext` and is promoted to `Offer.previews[]` at discovery.
