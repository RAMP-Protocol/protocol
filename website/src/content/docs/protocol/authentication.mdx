...
The protobuf body continues to carry identity semantics — `Requester` (who is making the request), scopes, and delegation tokens. What moves to HTTP is the **cryptographic proof** that these identities are authentic, including the forwarding chain, which is now a stack of labeled RFC 9421 signatures rather than an in-message hop list (see [Multi-hop forwarding](#multi-hop-forwarding)).
