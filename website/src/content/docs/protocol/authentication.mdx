...
|---|---|---|---|---|
| subject / holder | `sub` (+ proof-of-possession) | key the token is sealed/attenuated to | **Yes** | Binds the grant to the key that signs the request. Without it the token is bearer-usable. |
...
| `DENIAL_REASON_DELEGATION_INVALID` | Biscuit delegation token expired |
