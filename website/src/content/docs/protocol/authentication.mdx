...
- The Exchange embeds `agent_identity_hash` — the [RFC 7638](https://www.rfc-editor.org/rfc/rfc7638) JWK Thumbprint (SHA-256) of the agent's Ed25519 request-signing key — inside the HMAC-signed URL, and echoes it on the response (`TransactionResponse.agent_identity_hash`, forwarded unchanged by the Broker on `DiscoveryResponse.agent_identity_hash`).
