...
The Exchange resolves a requester's public key by fetching its WBA directory — the JWK Set at `{domain}/.well-known/http-message-signatures-directory` — and selecting the `JsonWebKey` whose RFC 7638 thumbprint matches the request signature's `keyid`. (The `{domain}` is the one announced in the requester's `WellKnownManifest` at `/.well-known/ramp.json` with `role=ROLE_AGENT`.)
...
2. Extracting `domain` from the `Requester` message and fetching its WBA directory at `{domain}/.well-known/http-message-signatures-directory`, selecting the `JsonWebKey` whose RFC 7638 thumbprint matches the `keyid` (and whose `[not_before, not_after)` window covers now)
...
Signature-Input: agent=("@method" "@target-uri" "content-digest");keyid="kPrK_qmxVWaYVA9wwBF6Iuo3vVzz7TxHCTwXBygrS4k";alg="ed25519"
...
- **keyid**: the RFC 7638 thumbprint of the agent's key — resolved in the agent's WBA directory at `.well-known/http-message-signatures-directory`
...
Content-Type: application/json
Content-Digest: sha-256=:X48E9qOokqqrvdts8nOJRJN3OWDUoyWxBf7kbu9DBPE=:
Signature-Input: agent=("@method" "@target-uri" "content-digest");keyid="kPrK_qmxVWaYVA9wwBF6Iuo3vVzz7TxHCTwXBygrS4k";alg="ed25519", broker=("@method" "@target-uri" "content-digest");keyid="broker-key";alg="ed25519"
