...
A manifest **may** additionally advertise where it serves that directory, through the optional [`identity_documents`](/protocol/exchange-manifest/#identity-documents) block. That is discovery metadata for a consumer reading the manifest. **It is not part of signature verification**: the verification steps below resolve the directory from the covered `Signature-Agent` header and the canonical path above, and they do not read the manifest at all.
