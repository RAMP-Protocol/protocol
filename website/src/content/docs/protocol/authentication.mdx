...
description: "Three-layer identity model: Requester identity with Ed25519 signatures, Biscuit delegation tokens for scoped authorization, and a stack of RFC 9421 HTTP Message Signatures for multi-hop request provenance."
---
...
Consider an enterprise where MarketData issues a JWT granting access to quotes and earnings data. An agent receives this JWT. When the agent delegates to a sub-agent that only needs earnings data, it cannot narrow the JWT — the sub-agent gets full quote access too. The only alternative is going back to MarketData for a new, narrower JWT.
...
```
user("marketdata.example.com");               // Who issued the delegation
...
| `token` | `bytes` | Token bytes — JWT (base64url) or Biscuit (binary, base64) |
| `token_format` | `string` | Token format: `"biscuit-v3"` (default) or `"jwt"` |
...
| `issuer` | `string` (optional) | Token issuer — OIDC issuer URL or GNAP grant server (for JWT/GNAP validation) |
| `ext` | `Struct` | Implementer-specific extensions |
| `ext_critical` | `repeated string` | Critical extension keys (COSE crit pattern) |
...
The `token` is opaque bytes, but its contents are the **authoritative** source of the grant (the plaintext `Delegation` fields are only the convenience mirror). For the caps and scopes to be verifiable and portable across Exchanges, RAMP defines a small **registered claim vocabulary** — the same registered-tokens + optional-fields + vendor-namespace pattern used elsewhere in the protocol. It applies to both formats (JWT claims / Biscuit facts):

| Claim | JWT | Biscuit fact | Required? | Meaning |
|---|---|---|---|---|
| subject / holder | `sub` + `cnf` (proof-of-possession key) | key the token is sealed/attenuated to | **Yes** | Binds the grant to the key that signs the request — verified per [Verification](#verification) step 3 (request-signing key MUST equal the bound key). Without it the token is bearer-usable. |
| issuer | `iss` | root key (`principal_domain`) | Yes | Who granted it. |
| `scope` | `scope` | `scope("…")` | No | Granted scopes (segment-wise; see [Scope Matching](#scope-matching)). |
| `exp` | `exp` | `expires("…")` | No (recommended) | Expiry. |
| `max_spend_cents` | `ramp_max_spend_cents` | `max_spend_cents(…)` | No | Spend cap (currency minor units). |
| `max_accesses` | `ramp_max_accesses` | `max_accesses(…)` | No | Access-count cap. |
| `quota_period` | `ramp_quota_period` | `quota_period_hours(…)` | No | Counter reset period. |

Rules:

- **Everything except the subject/holder binding is optional.** An issuer that only wants scope + expiry sets just those; absent caps mean "no cap from the delegation" (the Exchange's own limits still apply).
- **Vendors MAY add namespaced claims** (`vendor:claim` / a vendor-prefixed fact).
- **Constraints fail closed.** A claim a verifier cannot evaluate is **binding by default** — it MUST reject rather than ignore (a dropped constraint would fail *open*, granting more than intended). This mirrors [`Restriction.advisory`](/protocol/licensing-terms/): the issuer may mark a custom claim *advisory* to downgrade it to ignorable. (Biscuit gets this for free — an unsatisfiable check fails verification; for JWT the issuer lists the mandatory-to-understand claims.)
- **`ramp_`-prefixed claim/fact names are reserved** for this registry. Vendors MUST use their own `vendor:` namespace and MUST NOT define `ramp_*` names, so registered claims can never be shadowed.

> **JWT in v1.** `token_format: "jwt"` is permitted on the wire, but **Biscuit v3 is the default and the only token format with a fully specified verification path in v1.** A complete JWT path — proof-of-possession via `cnf` ([RFC 7800](https://datatracker.ietf.org/doc/html/rfc7800)) bound with DPoP or mTLS, and issuer discovery via OIDC→JWKS — is **deferred past v1**. Until then, an Exchange MAY accept JWT delegations only if it can enforce the **same mandatory holder binding** (step 3) and fail-closed constraint rules; otherwise it MUST reject `token_format: "jwt"` rather than fall back to bearer semantics. There is no degraded mode.

### Attenuation Example

MarketData issues a Biscuit for an agent with broad access:

```
// Authority block (signed by marketdata.example.com)
user("marketdata.example.com");
scope("quote:*");
scope("earnings:*");
...
**Zero round trips to MarketData.** Each level of delegation is local. The token encodes the full permission chain.
...
3. **Verifying holder binding** — the key that signed the HTTP request ([RFC 9421](#request-signatures)) MUST equal the key the token is sealed/attenuated to (the holder key in the final block). If they differ, **reject** (`DENIAL_REASON_DELEGATION_INVALID`). This step is what makes a leaked token useless to a thief: possessing the token bytes is not enough — you must also control the request-signing key the token is bound to. It is **not optional** and **not skippable**; an Exchange that omits it reduces the token to a bearer credential and breaks the anti-theft guarantee.
4. Evaluating all check conditions across all blocks
5. Extracting effective scopes (intersection of all blocks)
6. Checking `expires_at` against current time
7. Optionally checking `revocation_uri` for high-value transactions
...
**Holder binding across hops.** When a holder attenuates and re-delegates (appends a block), the new block re-seals the token to the *sub-agent's* key, so at every hop the bound key is the key of whoever now signs the request. The chain of Ed25519 block signatures is what carries the binding forward — there is no point at which the token reverts to bearer form.
...
### Subscription Agent (MarketData Biscuit)

A financial analysis agent carries a MarketData-issued Biscuit that grants access to subscription-tier data.
...
2. Deserializes the Biscuit `token`, verifies the authority block is signed by `marketdata.example.com`'s key
