...
Multi-hop is the common case, not the exception. A request typically travels `Agent -> Broker -> ... -> Exchange`, and the broker layer may itself be more than one hop. Each forwarding party appends an `IntermediaryHop` to the `intermediaries` array and authenticates its hop with its own [RFC 9421](https://datatracker.ietf.org/doc/html/rfc9421) HTTP Message Signature (one `Signature` / `Signature-Input` entry per hop). The Exchange verifies every hop by fetching each intermediary's key from `{hop.domain}/.well-known/ramp.json`. `IntermediaryHop.signature_label` optionally names the exact signature entry a hop added, so a verifier matches a hop to its signature without inferring by ordinal position or domain.

**Chain-depth caps.** Two fields bound how deep a chain may go:

- `RequestConstraints.max_hops` -- the agent caps its own chain depth. A Broker MUST NOT forward a request whose chain would exceed it.
- `WellKnownManifest.max_intermediary_hops` -- the Exchange publishes the longest inbound chain it tolerates; longer chains SHOULD be rejected, and a Broker can prune before forwarding.
...
```json
{
  ...
  "acceptable_restrictions": [{ "axis": "FUNCTION", "values": ["FUNCTION_AI_INPUT"] }],
  "intermediaries": []
}
```

Key fields:
- `requester.id` + `requester.domain` -- uniquely identifies the agent; the Exchange fetches the Ed25519 public key from `{domain}/.well-known/ramp.json` (`role=ROLE_AGENT`) to verify the signature
...
- request authentication -- an RFC 9421 HTTP Message Signature over the request (the agent's key is published at its `ramp.json`)
...
```json
{
  "offers": [
    ...
    },
    {
      ...
      "terms": [ { "scopes": ["subscription:SUB-12345"] } ],
...
The Exchange resolves most disputes automatically in under 1 second using CDN logs and cryptographic evidence. See [Dispute Resolution](/protocol/dispute-resolution/) for the three-tier resolution system, auto-credit rules, and the complete dispute lifecycle.
