...
- An Ed25519 `Offer.signature` that proves the Exchange issued this offer
...
4. Verify `offer.signature` over the presented bytes of the reflected Offer, then its `expires_at` freshness (stateless — no offer storage)
