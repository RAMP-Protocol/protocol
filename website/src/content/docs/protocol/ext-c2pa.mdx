...
- [C2PA Specification 2.3](https://spec.c2pa.org/specifications/2.3/specs/C2PA_Specification.html) (recommended, not required for consumers)
...
2. **Term layer** (RAMP): The Exchange maps the C2PA declaration onto `LicenseTerm.restrictions` that ride on the offer. The agent self-selects the term it can honour and bears compliance (enforced downstream at accept → report → reconcile). As a convenience, an Exchange MAY pre-filter — an agent that volunteers `acceptable_restrictions: [{ axis: "RESTRICTION_KIND_FUNCTION", values: ["ai-train"] }]` on the query can ask to skip offers for content where training is prohibited so it doesn't pay to discover them — but that filter is advisory, not an enforcement guarantee.
...
| Signature format | COSE Sign1 (RFC 9052) | Detached Ed25519 over RFC 8785 JCS, hex-encoded |
...
COSE Sign1 signature    ──►        Detached Ed25519 (hex)
...
|-----------|-------------|-------------|
| Ed25519 | Yes (EdDSA via COSE) | Yes (detached EdDSA, hex) |
| ECDSA P-256 | Yes | No |
| RSA | Yes | No |
