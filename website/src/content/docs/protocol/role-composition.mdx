...
| Third-party exchange across non-O&O supply | `ROLE_EXCHANGE` | `marketplace.example.com` |
...
2. **Cryptographic transaction boundaries.** Every `Offer` is signed by the Exchange (`exchange_signature`); every `TransactionRequest` is signed by the Agent (and by every intermediary in the `IntermediaryHop` chain). A Broker run by the same operator as an Exchange cannot suppress or rewrite this signature trail without producing a verifiable forgery.
