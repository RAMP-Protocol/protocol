...
### DiscoveryRequest.supported_profiles (field 5)
...
- **Keys listed in `ext_critical`**: the consumer **MUST** understand the key's semantics or reject the message. Rejection uses `OFFER_ABSENCE_REASON_UNKNOWN_CRITICAL_EXTENSION`.

### Example

```json
{
  "offer_id": "97e104b2-72cc-4401-aa56-e1887a1f491c",
  "ext": {
...
**Scope:** CoMP (Content Monetization Protocols, V1) metadata -- optional extension for Exchanges that carry CoMP-originated content objects

| Property | Detail |
|---|---|
| **Ext fields** | CoMP `Package`, `Scope` (licensing folded into Scope in V1), `Retrieval`, and media-type objects, carried as flat `comp.<field>` scalar keys in the `ext` Struct |
| **Standards** | [CoMP V1](https://github.com/IABTechLab/CoMP/blob/880238e0100b3d0d67d5afd7357a18fc21a97be5/CoMP-1.0.md) (finalized 2026-04-28; spec-file blob `aa8c796be7a9bcfa1189a1cf6c1ec50aa67a0f5f`) |
| **Key capabilities** | Structured content metadata (Package, Scope, Text, Image, Video, Audio); retrieval method negotiation; licensing and rights modeling via `Scope` (`ause`, `pricetype`, `pricetier`, `unitprice`, `licensedur`) |
| **Conformance levels** | Basic (Package fields), Standard (Scope + media types), Full (Scope licensing + Retrieval) |

CoMP metadata is carried as flat `comp.<field>` scalar keys in the `ext` Struct on protocol messages — not as a nested `comp` object. Exchanges that support IAB CoMP include keys such as `comp.package_id`, `comp.scope_type`, and `comp.allowed_use` (see [ext-comp](/protocol/ext-comp/) for the authoritative key list). Exchanges that don't use CoMP simply omit these keys, and consumers that don't understand CoMP safely ignore them.
...
All profile-specific metadata flows through the `google.protobuf.Struct ext` field present on every major protocol message: `ResourceQuery`, `Offer`, `ResourceIdentity`, `ResourceEntry`, `ResourceAttestation`, `DisputeRequest`, `WellKnownManifest`, and `UsageReport`.
