...
The catalog is the hot data structure. It must support fast lookup by URI (exact match and prefix/glob), filtered by access restrictions and AI system function.
...
type OfferTemplate struct {
    ...
    Restrictions  *rampv1.AccessRestrictions
    DeliveryMethod rampv1.DeliveryMethod

    // For fast filtering during DiscoverResources.
    PermittedFunctions map[compv1.Function]bool
    ProhibitedFunctions map[compv1.Function]bool
}
```
...
type TransactionRecord struct {
    ...
    // Who purchased.
    BuyerLicense string `json:"buyer_license"` // Last 8 chars only in logs
    ...
    // Agent identity hash (SHA-256 of license ID). Non-optional string, not pointer.
...
Cumulative benchmarking (same content, different agents, compare reported token counts) is a Exchange implementation concern, not a protocol requirement. The protocol accepts that token accuracy is a known limitation — agents self-report, and the Exchange's token estimate is itself approximate (word_count x 1.32).
...
- `agent_id` — hashed agent identity (`SHA256(lid)`)
- `timestamp` — when the transaction was executed

**Verification capabilities**:
- Provider can independently verify every Offer's `exchange_signature` using the Exchange's published Ed25519 public key (announced at `{exchange_domain}/exchange/v1/keys`)
...
type DisputeRecord struct {
    ...
    BillingID       string        `json:"billing_id"`
    BuyerLicense    string        `json:"buyer_license"`
    TenantID        string        `json:"tenant_id"`
...
**Storage**: Dispute records share the same durable store as transaction records (PostgreSQL for Growth tier, ClickHouse for Scale tier). They are indexed by `dispute_id`, `transaction_id`, and `(buyer_license, filed_at)` for compliance queries.
