...
sequenceDiagram
    ...
    Note over Handler: Extract tenant from requester.uris[0] domain
    Note over Handler: (Volumetric rate limiting handled by infra layer — HAProxy/Envoy)

    Handler->>SD: FindOffers(ctx, tenantID, uris, requester.scopes)

    Note over SD: Lookup URI(s) in radix trie
    Note over SD: Filter catalog by requester.scopes
    Note over SD: Filter by AccessRestrictions vs intended_use
...
func (h *ExchangeHandler) DiscoverResources(
    ...
    //    Lookup the requester's public key via {domain}/.well-known/ramp-agent.json.
    ...
    //     Each hop's signature is verified against {hop.domain}/.well-known/ramp-agent.json.
    ...
    // 3. Resolve tenant
    tenantCfg, err := h.tenants.ResolveFromURIs(requester.Uris)
    if err != nil {
    ...
    offers, err := h.catalog.FindOffers(ctx, tenantCfg.ID, requester.Uris, effectiveScopes)
    if err != nil {
    ...
    //    When a single URI is queried, use the flat offers field for backwards compat.
    uris := requester.Uris
    if len(uris) > 1 {
...
Requester authentication uses Ed25519 signatures over a domain-bound key. The `Requester.domain` field determines where the public key is published (`{domain}/.well-known/ramp-agent.json`). The `Requester.id` identifies the specific entity within that domain. The requester's private key never leaves the requester.
...
2. **Self-signup (domain verification)**: The requester registers via the public API: `POST /exchange/v1/requesters/register { requester_id, domain }`. The Exchange fetches `/.well-known/ramp-agent.json` from the domain, extracts the public key, and verifies that `requester_id` matches. No subscription is created — self-signup requesters use per-request billing only.
...
type RequesterRecord struct {
    ...
    ManifestURL   string  // {domain}/.well-known/ramp-agent.json
...
1. **Verify `Requester.signature`** — proves the original requester authorized the request. The Exchange looks up the requester's public key via `{requester.domain}/.well-known/ramp-agent.json` and verifies the Ed25519 signature over `(id, domain, uris, scopes)`.

2. **Verify each `IntermediaryHop.signature`** — proves each intermediary in the chain (e.g., Broker) handled the request. For each hop, the Exchange fetches the public key from `{hop.domain}/.well-known/ramp-agent.json` and verifies the Ed25519 signature. Timestamps (`forwarded_at`) are checked for temporal consistency.
...
sequenceDiagram
    ...
    participant Manifest as ProviderManifest Cache
    participant Verifier as ramp-verifier.json Cache
    ...
    loop For each ResourceEntry with attestations
        ...
        Verifier-->>Handler: ramp-verifier.json (JWKS)
