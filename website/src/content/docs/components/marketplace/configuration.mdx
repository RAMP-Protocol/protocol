...
|---|---|---|---|
| Listen address | Env var | No (restart) | `MARKETPLACE_LISTEN_ADDR=:8080` |
| Exchange domain | Env var | No | `MARKETPLACE_DOMAIN=exchange.ssp.com` |
| TLS cert/key | Env var / file | No | `MARKETPLACE_TLS_CERT=/etc/tls/cert.pem` |
| Tenant configs | Config file (YAML/JSON) | Yes (SIGHUP) | `MARKETPLACE_CONFIG=/etc/exchange/config.yaml` |
| Signing keys | Secrets manager | Yes (polled) | `MARKETPLACE_SECRETS_PROVIDER=aws-sm` |
| Billing adapter endpoint | Config file | Yes (SIGHUP) | In tenant config |
| Log level | Env var | Yes (signal) | `MARKETPLACE_LOG_LEVEL=info` |
| WAL directory | Env var | No | `MARKETPLACE_WAL_DIR=/var/lib/exchange/wal` |
| Transaction store DSN | Env var | No | `MARKETPLACE_TXN_STORE_DSN=kafka://...` |
