...
| **Attestation Verification (v1.0)** | Verifies `ResourceAttestation` signatures at catalog push time using keys from `ramp-verifier.json`. Propagates valid attestations to offers |
...
| **[Exchange Manifest](/protocol/exchange-manifest/)** | Machine-readable self-description at `/.well-known/ramp-exchange.json`. Declares endpoints, capabilities, accepted verifiers, `supported_profiles`, and `base_currency`. Enables Broker capability negotiation and cross-exchange price comparison |
...
The Exchange Node has five internal modules. Each module owns a distinct responsibility. Modules communicate via Go interfaces — no internal RPC, no message queues, no shared mutable state between modules. Identity verification uses the `Requester` model (Ed25519 domain-bound keys) with optional Biscuit delegation and `IntermediaryHop` chain verification.
...
1. An AI agent discovers available resources by calling `DiscoverResources` — the Exchange verifies the `Requester` identity (Ed25519 signature over domain-bound key), filters the catalog by the requester's scopes, and returns signed offers. If a Biscuit `Delegation` is present, the Exchange verifies the delegation chain before granting scoped access. When `intermediaries` is populated, each `IntermediaryHop` signature is verified to prove the request path
