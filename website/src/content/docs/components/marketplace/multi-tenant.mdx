...
A single Exchange Node serves many providers. This is the multi-tenant model — one Exchange operator runs a Exchange that serves 10, 100, or 1000+ providers. Each provider is represented as a "tenant" with isolated configuration, catalog, signing keys, and reporting obligations.
...
```go
// Resolve tenant from ResourceQuery URIs
tenantCfg, err := h.tenants.ResolveFromURIs(req.Msg.Aisystem.Aisysuse.Uri)
...
The edge function expects to fetch `ramp.json` (ProviderManifest) and `rsl.txt` from the Exchange on behalf of provider domains. The Exchange generates these from its tenant catalog and serves them for edge caching.
...
Returns a generated `ProviderManifest` for the given domain. The manifest is built from the tenant's catalog configuration (pricing, access restrictions, exchange endpoint). Cached by the edge function with a TTL matching the tenant's `rsl_poll_interval`.
...
```
GET /exchange/v1/keys
```

Returns the **Exchange's** Ed25519 public keys for **offer signature** verification. Brokers and agents fetch this to verify `exchange_signature` on Offers without manual key exchange.

**Note:** This endpoint serves the Exchange's offer-signing public key. Agent keys are separate — they are fetched from the agent's `/.well-known/ramp-agent.json` manifest URL (self-signup) or stored from enterprise registration (out-of-band key exchange).

Since the default offer signature algorithm is Ed25519, these are **public keys** — this endpoint does NOT require authentication. Public keys are safe to expose; they can only verify signatures, not create them.
...
- Returns current and previous keys (for rotation grace period)
...
- The `public_key` field contains the Ed25519 public key (32 bytes, base64-encoded)
...
1. **Verify `caller_signature`**: The Exchange looks up the registered Ed25519 public key for `caller_id` and verifies the signature over the request payload. Invalid signatures are rejected with `CodeUnauthenticated`.
