...
type BillingAdapter interface {
    ...
    GetBalance(ctx context.Context, buyerLicenseID string) (*BillingBalance, error)
    ...
    CheckSubscription(ctx context.Context, buyerLicenseID string, tenantID string) (*SubscriptionStatus, error)
}
...
type BillingAuthorizeRequest struct {
    BuyerLicenseID string  // The agent's license ID
...
type InMemoryBillingAdapter struct {
    mu       sync.RWMutex
    balances map[string]float64 // license ID -> balance
...
func (a *InMemoryBillingAdapter) Authorize(
    ...
    balance, ok := a.balances[req.BuyerLicenseID]
    if !ok {
    ...
    a.balances[req.BuyerLicenseID] -= req.Amount
...
// Table schema:
//   PK: "BUYER#<license_id>"  SK: "BALANCE"     → {available, currency, credit_limit}
//   PK: "BUYER#<license_id>"  SK: "TXN#<ulid>"  → {billing_id, amount, ...}
...
func (a *ExampleSSPBillingAdapter) Authorize(
    ...
    sspReq := &ssp.AuthorizeRequest{
        AccountID: mapLicenseToAccount(req.BuyerLicenseID),
        Amount:    req.Amount,
...
|---|---|---|
| `per_article` | Fixed price per article | unit_cost comparison |
| `per_crawl` | Fixed price per crawl session | unit_cost comparison |
| `per_token` | Actual tokens consumed (reported) | unit_cost comparison + initial estimate |
| `per_page` | Fixed price per page | unit_cost comparison (legal, court filings) |
| `per_minute` | Price per minute of content | unit_cost comparison (video, audio, streaming) |
| `per_record` | Price per record accessed | unit_cost comparison (database records, credit reports) |
| `subscription` | Flat rate per period | Not applicable |
| `free` | No charge | Not applicable |
| `attribution` | Free with mandatory citation | Not applicable |
| `contribution` | Revenue share on downstream output | unit_cost comparison |
| `training` | Bulk licensing for model training | Not applicable |
...
`word_count` comes from RSL content metadata or sitemap `<content:wordcount>` extensions. If unavailable, the Exchange estimates from content length: `word_count ~ content_length_bytes / 5.5`. For non-token pricing models (per_page, per_minute, per_record), the `estimated_quantity` is derived from content metadata specific to the unit type.
...
type ExchangeConfig struct {
    ...
    Currency string // ISO 4217 — e.g., "USD". All offers use this currency.
                    // Published as base_currency in ExchangeManifest.
}
```
...
Revenue tracking and reconciliation are handled through the transaction log. The `BillingRecordRequest` includes all fields needed for settlement: `BillingID`, `TransactionID`, `Amount`, `Currency`, `TenantID`, `BuyerLicense`, `ContentURI`, and `Timestamp`.
