...
The Exchange does not know or care whether the request came from an Broker or directly from an agent. The protocol is identical.
...
|---|---|---|
| **Exchange Registry** | Knows which Exchanges exist and their health status. Bootstraps from [Exchange Manifest](/protocol/exchange-manifest/) (`/.well-known/ramp-exchange.json`) instead of HEAD probes -- extracting endpoints, capabilities, and accepted verifiers in a single fetch | Long-lived, refreshed periodically |
...
## Request Signing and Intermediary Chain

RAMP uses a two-layer signing model: the **Requester signature** proves who originated the request, and the **intermediary chain** proves which intermediaries forwarded it.

### Requester Signature

The `Requester` message carries an Ed25519 signature over `(id, domain, uris, scopes)`. This proves that the entity claiming to be `requester.id` at `requester.domain` actually authored the request. The Exchange verifies this signature against the public key published at `{requester.domain}/.well-known/ramp-agent.json`.

The Broker passes `Requester` through unchanged -- it never modifies the requester's identity or signature.

### Intermediary Chain (schain-inspired)

When the Broker forwards a request, it appends a signed `IntermediaryHop` to the `ResourceQuery.intermediaries` chain. Each hop records:

- **`domain`** -- the intermediary's canonical domain (for public key lookup)
- **`id`** -- the intermediary's identifier
- **`forwarded_at`** -- timestamp of when the intermediary forwarded the request
- **`signature`** -- Ed25519 signature over the request payload at this hop

This chain is modeled after OpenRTB schain (Supply Chain Object). When the agent queries a Exchange directly, `intermediaries` is empty. When the request passes through an Broker, it contains one hop.

```go
// Broker appends a signed hop when forwarding.
func (o *Broker) signHop(query *rampv1.ResourceQuery) {
    hop := &rampv1.IntermediaryHop{
        Domain:      o.domain,                     // e.g., "broker.example.com"
        Id:          o.id,
        ForwardedAt: timestamppb.Now(),
        Signature:   o.signPayload(query),          // Ed25519 over the query payload
        SignatureAlgorithm: "ed25519",
    }
    query.Intermediaries = append(query.Intermediaries, hop)
}
```
...
1. **Requester signature** -- verified against the public key at `{requester.domain}/.well-known/ramp-agent.json`
2. **Each intermediary hop signature** -- verified against the public key at `{hop.domain}/.well-known/ramp-agent.json`

Both must pass. This replaces the earlier flat `agent_signature` + `broker_signature` model with a chain that generalizes to multiple intermediaries.

The Broker announces its public key at `{domain}/.well-known/ramp-agent.json` (same manifest format as agents). This allows any Exchange to verify the Broker's identity without prior registration.
...
2. **Broker filters Exchanges** -- the Exchange Registry checks each Exchange's `ExchangeManifest.supported_profiles` and routes queries only to Exchanges that support at least one of the requested profiles
...
The agent already knows which resources it wants. It sends `Requester.uris` with one or more specific content identifiers (DOIs, URLs, etc.). The Broker forwards these as a `ResourceQuery` to each Exchange and collects offers.
...
// v1: only ExchangeSource (queries a Exchange).
...
sequenceDiagram
    ...
    Note over Orch: Selection Engine:<br/>5 offers received<br/>4 pass AccessRestrictions<br/>3 pass RequestConstraints<br/>2 after dedup (1 AP article from both)<br/>Rank by unit_cost: MP-A offer wins
...
| **Ed25519 + intermediary chain** | Requester signs (id, domain, uris, scopes); Broker appends a signed `IntermediaryHop`. Exchange verifies both layers. Public keys discoverable via `.well-known/ramp-agent.json`. |
...
```
github.com/postindustria-tech/ramp-protocol/broker/
    cmd/
