...
The Selection Engine operates on offers, not on how those offers were discovered. Whether the agent sent specific URIs (`Requester.uris`), structured search filters (`RAMPRequest.search_filters`), or a natural-language query (`RAMPRequest.query`), the Broker resolves all three into a set of offers from Exchanges. The Selection Engine then applies the same pipeline -- filter, deduplicate, rank, verify -- to every offer identically.
...
func (d *Discovery) FanOut(
    ...
    for i, mp := range exchanges {
        i, mp := i, mp
        g.Go(func() error {
            query := toResourceQuery(req, mp) // includes all req.Aisystem.Aisysuse.Uri
            start := time.Now()
...
```mermaid
graph LR
    A[Collect all Offers<br/>from N Exchanges] --> B[Filter by<br/>AccessRestrictions]
...
## Stage 1: Filter by AccessRestrictions

Each Offer carries optional `AccessRestrictions` from the provider's RSL. The Broker filters offers that don't match the agent's declared usage.

```go
func filterByRestrictions(offers []rankedOffer, aisystem *compv1.AISystem) []rankedOffer {
    var passed []rankedOffer
    for _, o := range offers {
        r := o.Offer.Restrictions
        if r == nil {
            passed = append(passed, o) // no restrictions = all permitted
            continue
        }

        // Check function compatibility
        if !functionsPermitted(r, aisystem.Aisysuse.Function) {
            continue
        }

        // Check geo restrictions (if agent declares geo)
...
func functionsPermitted(r *rampv1.AccessRestrictions, requested []compv1.Function) bool {
    // If prohibited_functions contains any requested function, reject
    for _, req := range requested {
        for _, prohibited := range r.ProhibitedFunctions {
            if req == prohibited {
                return false
            }
        }
    }
    // If permitted_functions is non-empty, requested must be a subset
    if len(r.PermittedFunctions) > 0 {
        for _, req := range requested {
            found := false
            for _, perm := range r.PermittedFunctions {
                if req == perm || perm == compv1.Function_FUNCTION_ALL {
                    found = true
                    break
                }
            }
...
The Broker SHOULD verify `exchange_signature` on the winning Offer before proceeding. This prevents a Exchange from tampering with its own offer fields after signing.

```go
func verifyOfferSignature(offer *rampv1.Offer, exchange string, pubKeys map[string]crypto.PublicKey) error {
    if offer.ExchangeSignature == "" {
        return fmt.Errorf("exchange_signature is required but empty")
    }
    ...
    return verifySignature(alg, key, payload, []byte(offer.ExchangeSignature))
}
```
