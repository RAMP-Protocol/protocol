...
The Broker fetches Exchange public signing keys from `GET /exchange/v1/keys`. Public keys are cached in Redis with a 24-hour TTL. On signature verification failure, the cache is refreshed before retrying.
