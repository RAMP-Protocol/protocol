...
The hosted Broker serves multiple AI companies. Tenant isolation is by API key or `license_id` prefix. Each tenant gets isolated: Exchange registry (different trust levels per tenant), budget tracking (per-tenant Redis keyspace), selection policy (per-tenant configuration), and selection logs.
...
func (m *multiTenantBroker) HandleRequest(
    ctx context.Context,
    req *connect.Request[rampv1.RAMPRequest],
) (*connect.Response[rampv1.RAMPResponse], error) {
    tenantID := tenantFromContext(ctx)
    orch := m.tenants.GetBroker(tenantID)
    return orch.HandleRequest(ctx, req)
}
```
...
# Budget defaults (overridden per-session by RAMPRequest)
budget:
...
| After ExecuteTransaction sent, before response | Exchange has charged but agent doesn't know | Agent retries with same `TransactionRequest.id` -- Exchange returns same response (idempotency) |
| After RAMPResponse returned | Agent has the signed URL | Agent tracks reporting obligations independently |
...
The Broker generates deterministic idempotency keys for `TransactionRequest.id`:
...
If the agent retries the same `RAMPRequest.id`, the Broker generates the same `TransactionRequest.id`, which the Exchange recognizes as a duplicate.
...
| **T20**: Kickback-driven selection | Broker has side deal with a Exchange | Selection rationale logged. Agent can spot-check by querying directly. |
...
The Broker fetches the Exchange's `/.well-known/ramp.json` and reads `public_keys` for offer-signing keys. Public keys are cached in Redis with a 24-hour TTL. On signature verification failure, the cache is refreshed before retrying.
...
| `broker_request_latency_seconds` | Histogram | Total RAMPRequest handling time |
| `broker_requests_total` | Counter | Total RAMPRequests handled |
