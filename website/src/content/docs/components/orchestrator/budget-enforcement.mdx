...
| **Per-period** | Redis, keyed by `license_id` or sub-account ID | Hard reject if period budget exhausted | Survives restart (persisted in Redis) | Redis |
...
Per-period budgets are keyed by `budget_scope` (or `license_id` if no scope is provided) and persisted in Redis with TTLs matching the period (e.g., monthly keys expire at month-end).
