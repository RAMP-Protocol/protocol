...
The Exchange verifies attestation signatures at push time (not at compilation time) using keys fetched from the verifier's `/.well-known/ramp-verifier.json`. Only valid, unexpired attestations with verified signatures are stored in the catalog.
