...
| `Permissions` | `ramp.AccessRestrictions` | `permitted_functions` | From RSL permits |
| `Prohibitions` | `ramp.AccessRestrictions` | `prohibited_functions` | From RSL prohibits |
...
The Exchange verifies attestation signatures at push time (not at compilation time) using keys fetched from the verifier's `/.well-known/ramp.json` `public_keys`. Only valid, unexpired attestations with verified signatures are stored in the catalog.
