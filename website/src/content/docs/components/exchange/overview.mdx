...
| **Signed URL Generation** | Per-tenant Ed25519 signed URLs, or CloudFront RSA where the CDN verifies natively, with configurable TTL and requester identity binding |
...
3. The agent fetches the resource using the signed URL — the delivery endpoint verifies the URL signature with the Exchange's published public key, holding nothing secret
