...
An Exchange implemented on the Go SDK mounts the service through `connectserver.NewCatalogServiceHandler`, which composes the same request-id · verify · validate · error-detail stack as the Exchange and Broker bindings: an unsigned push never reaches the handler, the wire tier (the `ResourceEntry` envelope rules and the `LicenseTerm` rules) refuses a malformed entry before it **on a mount that passes `connectserver.WithValidation(connect.ValidationStrict)`** — that option composes protovalidate and is not the default, so a deployment relying on the boundary rules names it explicitly — and the proven signer is available to the handler from the request context. Steps 2 and 3 above — contributor authorisation and tenant binding — and the ingest-tier term checks (`helpers.ValidateResourceEntry`) are the handler implementation's job.
