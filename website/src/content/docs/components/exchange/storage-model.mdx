...
**The transaction log is not append-only, and `REVOKE UPDATE` on it would break the write path.** A transaction row is updated after it is written: when the usage report for that transaction lands, the consumed quantity is recorded on the row. The log is the *operational* record and it tracks a transaction through its lifecycle, so revoking `UPDATE` would block a legitimate write the protocol requires.

Append-once belongs to the **evidence** store described below, and only there. That is the distinction: the log follows a transaction as it changes, the evidence row states what the two parties agreed to and can never legitimately change afterwards. Revoke `UPDATE` on that table, not on this one.

`DELETE` is revoked on both. Retention is enforced by dropping whole partitions after export, which does not need row-level `DELETE`.
...
The row is **append-once**: written once during `ExecuteTransaction`, then never updated and never deleted before its retention limit. Enforce it with `REVOKE UPDATE, DELETE` on this table. Note that this is *stricter* than the transaction log above, which is updated when a usage report lands — append-once is a property of the evidence store alone, and the difference is the point: the log follows a transaction as it changes, this row states what was agreed and can never legitimately change afterwards. Its only read path is the operator plane's `GetTransactionEvidence` RPC.
...
**This is not the same key as `query_id`.** The `ResourceQueryReceived` event records a `query_id`, which is the `X-Request-ID` of a *discovery* request. The evidence row's `request_id` is the `X-Request-ID` of an *execute* request. Different legs of the exchange, different requests, different values — they must not be joined against each other or read as one correlation chain. The evidence-plane id joins outward to whatever else recorded the same correlation id for that execute call: the Exchange's own request logs, and any tracing or log pipeline that saw the header. It does **not** join to the edge delivery log — the delivery fetch is a separate HTTP request made later by the agent to the CDN, and it does not carry the execute request's `X-Request-ID`. The join to the delivery side is `signed_url_hash`.
