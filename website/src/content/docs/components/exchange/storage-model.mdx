...
- `offer_snapshot` — full Offer at transaction time, including `Offer.signature`
...
- Provider can independently verify every Offer's `Offer.signature` using the Exchange's published Ed25519 public key (fetch the Exchange's WBA directory, the JWK Set at `/.well-known/http-message-signatures-directory`, and read its `keys`)
