...
func (h *ExchangeHandler) DiscoverResources(
    ...
    //    The signing key lives in the signer's WBA directory at
    //    {domain}/.well-known/http-message-signatures-directory, and the domain
    //    comes from the COVERED `Signature-Agent` header — the self-asserted
    ...
    keyDomain, err := coveredSignatureAgent(req) // from the signed header, not the body
    if err != nil {
        return nil, connect.NewError(connect.CodeUnauthenticated, err)
    }
    requesterPubKey, err := h.requesterKeys.LookupPublicKey(ctx, keyDomain)
    if err != nil {
        return nil, connect.NewError(connect.CodeUnauthenticated,
            fmt.Errorf("no key directory for domain: %s", keyDomain))
    }
    ...
    //     verifyForwardingChain resolves each keyid (an RFC 7638 thumbprint)
    //     in the key set the Exchange is configured with — Signature-Agent
    //     stays the agent's directory, so hop keys are not discovered through
    //     it — verifies every signature, and confirms the chain is contiguous.
    ...
    //     the covered directory domain plus the signing key's RFC 7638
    //     thumbprint. The self-asserted requester.id/domain never select an
    //     account — they are display fields.
    account, err := h.accounts.FromVerifiedKey(ctx, keyDomain, thumbprint(requesterPubKey))
    if err != nil {
        return nil, connect.NewError(connect.CodePermissionDenied, err)
    }
...
func (h *ExchangeHandler) ExecuteTransaction(
    ...
    // 2. Verify Requester identity (RFC 9421 HTTP Message Signature in the HTTP
    //    headers, Ed25519, domain-bound key). As in DiscoverResources step 2,
    //    the key domain comes from the COVERED `Signature-Agent` header, never
    //    from the self-asserted Requester.domain.
    requester := req.Msg.Requester
    keyDomain, err := coveredSignatureAgent(req)
    if err != nil {
        return nil, connect.NewError(connect.CodeUnauthenticated, err)
    }
    requesterPubKey, err := h.requesterKeys.LookupPublicKey(ctx, keyDomain)
    ...
    //     (covered directory domain + key thumbprint), as in DiscoverResources
