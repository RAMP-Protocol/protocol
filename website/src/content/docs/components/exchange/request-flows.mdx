...
func (h *ExchangeHandler) DiscoverResources(
    ...
    //    Lookup the requester's public key in the WBA directory at
    ...
    //     verifyForwardingChain resolves each keyid in that hop's WBA directory
    //     ({domain}/.well-known/http-message-signatures-directory),
...
2. **Verify every forwarding signature in the stack** — proves each intermediary (e.g., Broker) handled the request. For each labeled signature, the Exchange resolves its `keyid` in that hop's WBA directory at `{domain}/.well-known/http-message-signatures-directory`, verifies the Ed25519 signature, and confirms it covers the prior hop's signature so the chain is contiguous and ordered.
...
//      {domain}/.well-known/http-message-signatures-directory.
//   3. For each subsequent labeled signature, resolves its keyid in that
//      hop's WBA directory, verifies the Ed25519 signature, and
