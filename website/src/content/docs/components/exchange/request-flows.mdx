...
1. **Verify `Requester.signature`** — proves the original requester authorized the request. The Exchange looks up the requester's public key via `{requester.domain}/.well-known/ramp.json` and verifies the Ed25519 signature over `(id, domain, uris, scopes)`.
