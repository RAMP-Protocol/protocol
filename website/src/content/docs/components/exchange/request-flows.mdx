...
Before the handler below runs, the wire tier has already refused a malformed push: the `ResourceEntry` envelope rules (bare-host `domain`, absolute `path`, bounded fields, at most 32 terms) and the `LicenseTerm` rules are protovalidate rules applied at the boundary. The handler then canonicalises the terms and applies registry membership — the ingest tier — before the attestation checks. Both tiers ship in the SDK as a publisher pre-check, so a conforming feed rarely reaches this flow with a refusable entry.
