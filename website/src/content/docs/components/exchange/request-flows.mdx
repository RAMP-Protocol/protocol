...
sequenceDiagram
    ...
    Note over Handler: Reject unless request.exchange names one of this Exchange's own domains (INVALID_ARGUMENT)
...
sequenceDiagram
    ...
    Note over Handler: Reject unless EVERY items[].offer.exchange names one of this Exchange's own domains (INVALID_ARGUMENT)
...
func (h *ExchangeHandler) ExecuteTransaction(
    ...
    for _, item := range req.Msg.Items {
        ...
        record := &TransactionRecord{
            ...
            // Derived PER-ITEM key: distinct items of a multi-item batch get
            // distinct keys, so neither the UNIQUE backstop nor billing dedup
            // collapses a batch. Applied unconditionally (single-item too).
            IdempotencyKey:    req.Msg.IdempotencyKey + ":" + offer.OfferId,
...
sequenceDiagram
    ...
    Note over Handler: Reject unless report.exchange names one of this Exchange's own domains (INVALID_ARGUMENT)
