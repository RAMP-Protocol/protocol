...
func (h *ExchangeHandler) DiscoverResources(
    ...
    //     delegation) — NOT from billing_ref, which only attributes cost.
...
sequenceDiagram
    ...
    Handler->>Idemp: Check(req.IdempotencyKey)
    alt Already processed
        Idemp-->>Handler: cached TransactionResponse
        Handler-->>Agent: TransactionResponse (replay)
    end

    Note over Handler: Verify offer.signature over the presented Offer bytes<br/>(stateless — no offer storage, no re-resolution)
    ...
    Handler->>Idemp: Store(req.IdempotencyKey, response)
...
func (h *ExchangeHandler) ExecuteTransaction(
    ...
    // 3. Idempotency check (TransactionRequest.idempotency_key — REQUIRED)
    ...
    // 4. Verify the reflected Offer (stateless — the request carries the FULL
    //    signed Offer; the Exchange verifies offer.signature over the presented
    //    bytes against its own key, and never rebuilds an offer from a catalog).
    //    A denied single transaction is NOT a success body: it travels as a
    //    non-OK transport error carrying ErrorDetail.transaction_denial (a typed
    //    DenialReason plus restriction_mismatches). The transport code is the
    //    error CLASS; the ErrorDetail is the typed reason.
    offer := req.Msg.Offer
    if err := h.offerSigner.VerifyPresented(offer); err != nil {
        if errors.Is(err, ramphelpers.ErrOfferExpired) {
            return nil, transactionDenied(rampv1.DenialReason_DENIAL_REASON_OFFER_EXPIRED)
        }
        return nil, transactionDenied(rampv1.DenialReason_DENIAL_REASON_SIGNATURE_INVALID)
    }
    ...
    if err != nil {
        return nil, transactionDenied(rampv1.DenialReason_DENIAL_REASON_CONTENT_UNAVAILABLE)
    }
    ...
    billingRef := requester.Id
    if requester.BillingRef != nil {
        billingRef = *requester.BillingRef
    }
    ...
    if !authResp.Approved {
        ...
        return nil, transactionDenied(denial)
    }
    ...
    record := &TransactionRecord{
        ...
        IdempotencyKey:    req.Msg.IdempotencyKey, // request's idempotency key
    ...
    // 11. Build response (correlation is not in the body — it rides the
    //     X-Request-ID HTTP header across the request/response round-trip)
    resp := &rampv1.TransactionResponse{
        Ver:           "1.0",
        TransactionId:     txnID,
        BillingId:         authResp.BillingID,
        ResourceTitle:     offer.Title, // human-readable title echoed from the Offer
        RetrievalEndpoint: proto.String(signedURL),
        Cost: &rampv1.Cost{
            Amount:   offer.Pricing.Rate,
            Currency: offer.Pricing.Currency,
    ...
    // 12. Cache for idempotency (keyed by TransactionRequest.idempotency_key)
...
// carrying ErrorDetail.transaction_denial. A denied single transaction is an
...
When `TransactionRequest.items` is populated, the Exchange processes each item independently. Batch transactions are **NOT atomic** — individual items can fail (signature invalid, resource unavailable, billing denied) while others succeed. Each `TransactionResultItem` carries its own `denial_reason` (and `restriction_mismatches`). Per-item denials stay **in-body** as partial results of a successful batch — unlike a denied *single* transaction, which is returned as a non-OK transport error carrying `ErrorDetail.transaction_denial`.
...
func (h *ExchangeHandler) executeBatchTransaction(
    ...
    for _, item := range req.Msg.Items {
        offer := item.Offer // REQUIRED — every batch item reflects its full signed Offer
        resultItem := &rampv1.TransactionResultItem{OfferId: offer.GetOfferId()}

        // 1. Verify the reflected Offer (per item — offer.signature over the
        //    presented bytes, then freshness)
        if err := h.offerSigner.VerifyPresented(offer); err != nil {
        ...
        record := &TransactionRecord{
            ...
            OfferID: offer.OfferId, TenantID: tenantCfg.ID,
            BillingRef: billingRef,
            Amount: amount, Currency: offer.Pricing.Currency,
        }
        if err := h.txLog.Write(ctx, record); err != nil {
            slog.Error("batch item txlog write failed", slog.String("offer_id", offer.OfferId), slog.Any("error", err))
            denial := rampv1.DenialReason_DENIAL_REASON_CONTENT_UNAVAILABLE
            resultItem.DenialReason = &denial
            resultItems = append(resultItems, resultItem)
            continue
