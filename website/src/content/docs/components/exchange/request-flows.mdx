...
func (h *ExchangeHandler) DiscoverResources(
    ...
    //    the SDK's verify middleware checked every RFC 9421 signature — the
    //    requester's and the whole forwarding chain (each labeled signature
    //    covers the request essentials plus the previous hop's signature, so
    //    the ordered header stack IS the chain) — resolving the requester's
    //    key by its signed keyid (an RFC 7638 thumbprint) in the WBA directory
    //    named by the COVERED `Signature-Agent` header. The self-asserted
    //    Requester.domain field never drives key lookup.
    ...
    verified := helpers.FromContext(ctx) // first (requester) signature; hops via helpers.AllSignaturesFromContext
    if verified == nil {
    ...
    // 2d. Resolve the caller's registered account from the VERIFIED identity:
    //     the covered directory (verified.SignatureAgent) plus the RFC 7638
    //     thumbprint of the key the signature actually verified against
    //     (verified.PublicKey — not the claimed keyid). The self-asserted
    //     requester.id/domain never select an account — they are display fields.
    keyThumb, err := helpers.Thumbprint(verified.PublicKey)
    if err != nil {
        return nil, connect.NewError(connect.CodeInternal, err)
    }
    account, err := h.accounts.FromVerifiedKey(ctx, verified.SignatureAgent, keyThumb)
    if err != nil {
        return nil, connect.NewError(connect.CodePermissionDenied, err)
    }
    ...
    //     from the caller-written requester.id.
    ...
    if subStatus != nil {
        ...
        for _, offer := range offers {
            ...
            subOffer.OfferId = "offer-" + ulid.Make().String()
...
Requester authentication uses an RFC 9421 HTTP Message Signature in the HTTP headers, signed with a domain-bound Ed25519 key. The public key is published in the signer's WBA directory (`{domain}/.well-known/http-message-signatures-directory`); the verifier resolves that domain from the covered `Signature-Agent` header. The `Requester.id` identifies the specific entity within that domain. The requester's private key never leaves the requester.
...
1. **Verify the requester's (originating) RFC 9421 HTTP Message Signature** — proves the original requester authorized the request. The `keyid` is the key's RFC 7638 thumbprint; the Exchange resolves the key in the requester's WBA directory at `{domain}/.well-known/http-message-signatures-directory` and verifies the Ed25519 signature.

2. **Verify every forwarding signature in the stack** — proves each intermediary (e.g., Broker) handled the request. For each labeled signature, the Exchange resolves its `keyid` thumbprint in the key set it is configured with (the covered `Signature-Agent` header stays the agent's directory, so a hop's key is not discovered through it), verifies the Ed25519 signature, and confirms it covers the prior hop's signature so the chain is contiguous and ordered.
...
```go
// Forwarding-chain verification runs in the SDK's verify middleware at the
// HTTP seam, before any handler — the ordered stack of RFC 9421 HTTP
// Message Signatures in the request headers IS the chain. The middleware:
//   1. Parses the Signature / Signature-Input headers into ordered, labeled hops.
//   2. Verifies the originating (requester) signature against the key in
//      the requester's WBA directory at
//      {domain}/.well-known/http-message-signatures-directory.
//   3. For each subsequent labeled signature, resolves its keyid in the
//      Exchange's configured key set, verifies the Ed25519 signature, and
//      confirms it covers the prior hop's signature (contiguous + ordered).
//   4. Counts the hops against max_hops / max_intermediary_hops.
// Any failure rejects the request with CodeUnauthenticated before the handler
// runs. The handler reads the proven hops from the context:
hops := helpers.AllSignaturesFromContext(ctx) // sig1..sigN, in chain order
```
...
func (h *ExchangeHandler) ExecuteTransaction(
    ...
    // 2. Requester identity. As in DiscoverResources step 2, the SDK's verify
    //    middleware already verified every RFC 9421 signature at the HTTP seam;
    //    the handler consumes the proof from the context and fails closed when
    //    it is absent.
    requester := req.Msg.Requester
    ...
    // 2b. Resolve the caller's registered account from the verified identity
    //     (covered directory + proven-key thumbprint), as in DiscoverResources
    //     step 2d. All entitlement, compliance, and billing below use this
    //     account, never the caller-written requester.id/domain.
    keyThumb, err := helpers.Thumbprint(verified.PublicKey)
    if err != nil {
        return nil, connect.NewError(connect.CodeInternal, err)
    }
    ...
    // account resolved in step 2b from the VERIFIED signing key. The request
    // carries no caller-written billing field, and the self-asserted id/domain
    // never select the account.
...
sequenceDiagram
    ...
    loop For each ResourceEntry with attestations
        ...
        alt Direct push (provider or vendor pushing own attestation)
            ...
            Note over Handler: Record which entry failed — the submission is refused whole
        end
        ...
        Note over Handler: Handler-applied claims-size limit (no wire rule bounds claims)
    end

    alt Every entry passed
        Handler-->>Caller: PushResourcesResponse { accepted: N }
    else Any entry failed
        Handler-->>Caller: non-OK error + ErrorDetail.catalog_rejection { reason, rejected_paths }
    end
```
...
**Rejection reasons** come from [`CatalogRejectionReason`](/reference/proto-ramp/#catalogrejectionreason). The ones this flow can produce are `CATALOG_REJECTION_REASON_NOT_CATALOG_CONTRIBUTOR` (the caller is not authorized for the domain), `CATALOG_REJECTION_REASON_SIGNATURE_INVALID`, `CATALOG_REJECTION_REASON_DOMAIN_NOT_VERIFIED` and `CATALOG_REJECTION_REASON_MALFORMED_ENTRY`, which the envelope rules make the common one. The attestation-specific failures — a bad attestation signature, an unknown verifier, oversized claims — have no dedicated value in that enum; they travel in the refusal's human-readable message.
