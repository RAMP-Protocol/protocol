...
- `agent_id` -- the agent's `agent_identity_hash` from `TransactionResponse`: the RFC 7638 JWK Thumbprint (SHA-256, base64url) of the agent's Ed25519 acceptance key. Because it is part of the HMAC input, a URL holder cannot swap the Exchange-issued value.
