...
<Aside type="note">
The TypeScript SDK ships in-repo and is CI-gated, but is not yet published to npm; the example below is illustrative.
</Aside>

```typescript
// npm package not yet published — illustrative
import { RampClient } from "@ramp-protocol/agent-sdk";
...
<Aside type="note">
The Python SDK ships in-repo and is CI-gated, but is not yet published to PyPI; the example below is illustrative.
</Aside>

```python
# PyPI package not yet published — illustrative
...
|---|---|---|---|
| Go | [`gen/go/`](https://github.com/RAMP-Protocol/protocol/tree/main/gen/go/) (types) + [`sdk/go/`](https://github.com/RAMP-Protocol/protocol/tree/main/sdk/go/) (the shipped hand-written SDK: `helpers`, `resolvers`, `core`, `connect`, `connectserver`) | Shipped and CI-gated | `connectrpc.com/connect` + `google.golang.org/protobuf` + stdlib |
| TypeScript | [`gen/ts/`](https://github.com/RAMP-Protocol/protocol/tree/main/gen/ts/) (types) + [`sdk/ts/`](https://github.com/RAMP-Protocol/protocol/tree/main/sdk/ts/) (hand-written SDK: `src`, `core`, `resolvers`, `client`, `hono`) | Shipped and CI-gated; npm package not yet published | `zod` + `undici` + `ajv` + `canonicalize` (`hono` optional) |
| Python | [`gen/python/`](https://github.com/RAMP-Protocol/protocol/tree/main/gen/python/) (types) + [`sdk/python/`](https://github.com/RAMP-Protocol/protocol/tree/main/sdk/python/) (hand-written SDK: `ramp_sdk`, `ramp_sdk.core`, `ramp_sdk.resolvers`, `ramp_sdk.client`, `ramp_sdk.sync`) | Shipped and CI-gated; PyPI package not yet published | `pydantic` + `httpx` + `cryptography` |

Every language also ships the account-setup role: `Register` and `GetAccountStatus` on the agent client (`Register`/`GetAccountStatus`, `register`/`getAccountStatus`, `register`/`get_account_status`), addressed by the Exchange domain on the request rather than by a configured origin, alongside the reader that fetches an Exchange's published registration requirements afresh on every read (`resolvers.NewWellKnownRequirementsReader`, `createWellKnownRequirementsReader`, `WellKnownRequirementsReader`). A payload the client's own schema pre-check refuses comes back carrying the same typed `RegistrationFailure` detail the Exchange would have attached, so one renderer serves both sides of the refusal.

Every language also ships the publisher role: a catalog client (`connect.NewCatalogClient`, `createCatalogClient`, `ramp_sdk.client.CatalogClient`) and the Exchange's two-tier entry pre-check (`helpers.ValidateResourceEntry`, `validateResourceEntry`, `validate_resource_entry`) — see [Source 7: CatalogService API Push](/components/content-ingestion/sources/#source-7-catalogservice-api-push).
