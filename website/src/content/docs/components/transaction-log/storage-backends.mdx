...
| Query pattern (reconciliation) | Batch, offline, range scans by time + provider + buyer |
| Query pattern (compliance) | Point lookup by buyer_lid (is this buyer blocked?) |
...
func (ps *PostgresStore) WriteBatch(events []*TransactionEvent) error {
    ...
    stmt, err := tx.Prepare(pq.CopyIn(
        ...
        "transaction_id", "billing_id", "request_id",
        "buyer_lid", "provider_domain", "exchange_id",
        "cost_amount", "cost_currency",
        "chain_hash", "exchange_signature",
        "payload",
    ))
    if err != nil {
        return err
    ...
    for _, event := range events {
        ...
        _, err := stmt.Exec(
            ...
            event.TransactionID, event.BillingID, event.RequestID,
            event.BuyerLID, event.ProviderDomain, event.ExchangeID,
            event.CostAmount, event.CostCurrency,
            event.ChainHash, event.ExchangeSignature,
            payload,
        )
        if err != nil {
            return err
...
type TransactionEvent struct {
    ...
    ExchangeSignature string `json:"exchange_signature"  db:"exchange_signature"`
