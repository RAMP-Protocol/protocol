...
func (ps *PostgresStore) WriteBatch(events []*TransactionEvent) error {
    ...
    stmt, err := tx.Prepare(pq.CopyIn(
        ...
        "chain_hash", "record_signature", "offer_id",
        "payload",
    ))
    if err != nil {
        return err
    ...
    for _, event := range events {
        ...
        _, err := stmt.Exec(
            ...
            event.ChainHash, event.RecordSignature, event.OfferID,
            payload,
        )
        if err != nil {
            return err
...
type TransactionEvent struct {
    ...
    // Layer-3 tamper-evidence: the Exchange's Ed25519 signature over
    // chain_hash (see Reconciliation — Signed Event Records). An event-record
    // signature, unrelated to the wire's Offer.signature.
    RecordSignature string `json:"record_signature" db:"record_signature"`
