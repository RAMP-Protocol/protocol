...
Every state transition in the transaction lifecycle produces a log event. There are eight event types, each corresponding to a protocol step. v1.0 adds two dispute-related event types: `DisputeRequested` and `DisputeResolved`.
...
graph TB
    ...
    subgraph "Side 2: Exchange Transaction Log"
        TXN[Transaction Log<br/>Every billing_id issued<br/>Fields: transaction_id, billing_id, cost, buyer, provider, signed_url_hash]
    end
...
- **Q4**: Buyer compliance status (real-time lookup, < 1ms)
- **Q5**: Transaction detail for dispute resolution
- **Q6**: Buyer spending summary for a period (invoicing)
