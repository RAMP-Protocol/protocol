...
### Q4: Buyer Compliance Status (Real-Time)

Fast point lookup: "Is this buyer blocked for non-compliance?" Called on every `ExecuteTransaction` to enforce reporting obligations. Must return in < 1ms.
...
Default compliance policy: block buyer if overdue_reports > 10 or overdue_rate > 20%. The 20% threshold is a protocol OPTIONAL default (RFC 2119 MAY). Individual Exchanges decide whether and how to enforce it.
...
| `GET /api/v1/reports/buyer-invoice` | Exchange operator | "How much do I bill buyer Y for period X?" | Q6 |
| `GET /api/v1/reports/compliance-status` | Exchange admin | Buyer reporting obligation status | Q3, Q4 |
| `GET /api/v1/reports/transaction-detail` | Buyer, Provider | Full event timeline for a transaction | Q5 |
...
func SignEvent(event *TransactionEvent, privateKey ed25519.PrivateKey) {
    ...
    event.ExchangeSignature = base64.StdEncoding.EncodeToString(sig)
}
```
...
| **Admin API** | Key rotation, provider reporting API, invoice PDF generation, buyer management | Exchange-specific (not standardized) |
