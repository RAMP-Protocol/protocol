...
type SelectionLog struct {
    // The agent's DiscoveryRequest.id this decision was made for (Broker-internal
    // audit key). Cross-service request/response correlation rides the
    // X-Request-ID HTTP header, not a message field.
