...
The Selection Engine operates on offers, not on how those offers were discovered. Whether the agent sent specific URIs (`DiscoveryRequest.uris`), structured search filters (`DiscoveryRequest.search_filters`), or a natural-language query (`DiscoveryRequest.query`), the Broker resolves all three into a set of offers from Exchanges. The Selection Engine then applies the same pipeline -- filter, deduplicate, rank, verify -- to every offer identically.
...
Uses `errgroup.Group` with a semaphore for bounded concurrency and `context.WithTimeout` for per-Exchange deadlines. When the `DiscoveryRequest` contains multiple URIs, the Broker sends all URIs in a single `ResourceQuery` to each Exchange (not one query per URI). The Exchange returns `OfferGroups` -- one per URI -- so the Broker knows which offers map to which content.
...
graph TD
    subgraph "Timeout Layers"
        ...
        T3["Layer 3: Request deadline<br/>agent's context deadline<br/>forwarded from DiscoveryRequest"]
    end
...
The agent's `DiscoveryRequest.constraints` specifies budget and preference limits.
...
## Stage 5: Verify Offer.signature

The Broker SHOULD verify the winning Offer's `signature` before proceeding. This prevents an Exchange from tampering with its own offer fields after signing.
...
func verifyOfferSignature(offer *rampv1.Offer, exchange string, pubKeys map[string]crypto.PublicKey) error {
    ...
    alg := "EdDSA" // the JOSE algorithm identifier for Ed25519; the signature
                   // itself is detached hex, not a JWS
...
type SelectionLog struct {
    // Broker-internal audit key correlating this decision to the inbound request.
    // Request/response correlation rides the X-Request-ID HTTP header, not a message field.
    RAMPRequestID       string              `json:"ramp_request_id"`
...
sequenceDiagram
    ...
    MP->>Orch: ResourceResponse (Offer with signature)
    Note over Orch: Broker CANNOT modify<br/>offer fields without<br/>invalidating signature
    Orch->>Agent: DiscoveryResponse (includes original signed Offer)
    Note over Agent: Agent verifies signature<br/>against MP's public key.<br/>Price tampering detected.
```

### Fee Disclosure

`DiscoveryResponse` carries the signed Offers unchanged (one `OfferGroup` per requested URI); it has no `broker_fee` field, so the Broker cannot fold a fee into the discovery result. Any Broker fee is additive and disclosed out of band, never hidden in the offer price — the Exchange's signature over the Offer's pricing fields makes silent inflation detectable. The agent sees: Exchange price + Broker fee = total cost.

### Audit Trail

For full auditability, the agent operator should:

1. **Verify offer signatures** on every DiscoveryResponse
