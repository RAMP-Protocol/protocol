...
2. **The forwarding signature stack** -- each labeled RFC 9421 signature is verified against the public key in that hop's WBA directory (`{hop-domain}/.well-known/http-message-signatures-directory`), and the chained coverage is checked hop-by-hop.
...
The Broker publishes its keys in its WBA directory (the JWK Set at `{domain}/.well-known/http-message-signatures-directory`) — RFC 7517 Ed25519 JWKs, each identified by its RFC 7638 thumbprint (the RFC 9421 `keyid`). Its `ramp.json` manifest uses the same format as agents (`role=ROLE_AGENT`). This allows any Exchange to verify the Broker's identity without prior registration.
