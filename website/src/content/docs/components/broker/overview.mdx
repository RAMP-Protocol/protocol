...
When the `Requester.delegation` carries a delegation token (JWT), the Broker MAY narrow it before forwarding. Delegation is a chain of `cnf`-linked JWTs -- the holder can issue a child JWT that adds restrictions (narrower scope, lower spend cap, fewer URIs) without going back to the issuer. This lets the Broker narrow a broad delegation for a specific Exchange:

- Original delegation: `scope: academic:*, max_spend_cents: 50000`
- Narrowed for Exchange A: `scope: academic:read, max_spend_cents: 10000, uris: doi:10.*`

The narrowed token is a new child delegation JWT appended to the chain (signed by the key the parent named in `cnf`, `scope ⊆ parent`). The Exchange verifies the full JWT chain back to the original principal.
