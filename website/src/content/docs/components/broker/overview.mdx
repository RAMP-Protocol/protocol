...
func (o *Broker) Resolve(ctx context.Context, req *rampv1.DiscoveryRequest) (*rampv1.DiscoveryResponse, error) {
    ...
    if err != nil {
        // "No viable offer" is a successful answer, not a transport error: return
        // OK with DiscoveryResponse.absence_reason set and no retrieval_endpoint
        // (mirrors DiscoverResources). Non-OK errors are reserved for malformed
        // requests, auth failures, and internal faults.
        return &rampv1.DiscoveryResponse{AbsenceReason: o.absenceReason(err)}, nil
    }
    ...
    // 6. Return DiscoveryResponse to agent
    return toDiscoveryResponse(req, txn, selection), nil
}
```
...
sequenceDiagram
    ...
    Agent->>Orch: DiscoveryRequest (aisystem + constraints)
