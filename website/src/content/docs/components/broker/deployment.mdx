...
The hosted Broker serves multiple AI companies. Tenant isolation is by API key or `billing_ref` prefix. Each tenant gets isolated: Exchange registry (different trust levels per tenant), budget tracking (per-tenant Redis keyspace), selection policy (per-tenant configuration), and selection logs.
