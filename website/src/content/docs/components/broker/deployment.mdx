...
| After DiscoveryResponse returned | Agent has the signed URL | Agent tracks reporting obligations independently |
...
If the agent retries the same `DiscoveryRequest.id`, the Broker generates the same `TransactionRequest.idempotency_key`, which the Exchange recognizes as a duplicate.
