...
If the agent retries the same `RAMPRequest.id`, the Broker generates the same `TransactionRequest.idempotency_key`, which the Exchange recognizes as a duplicate.
