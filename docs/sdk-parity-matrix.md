...
| `helpers.HostAnchored` | Go label-boundary same-host predicate, port-insensitive. Python and TS carry a PRIVATE near-namesake in their WBA modules, but it is a port of the resolvers-tier wbaHostAnchored and compares WITH the port, so it is not a counterpart to this one; exporting an aligned predicate in all three is tracked separately. |
