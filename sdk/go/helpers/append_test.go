...
func mustSigner(t *testing.T, keyID string) (helpers.Signer, ed25519.PublicKey) {
	t.Helper()
...
func TestAppendSignature_EmptyHeaders(t *testing.T) {
	...
	v, err := helpers.VerifyRequestResolved(ctx, req, body, resolver, helpers.VerifyOptions{Now: now})
	if err != nil {
...
func TestAppendSignature_ByteIdenticalToSignRequest(t *testing.T) {
	...
	signerA, err := helpers.NewEd25519SignerFromSeed("agent.test", seed)
	if err != nil {
		t.Fatal(err)
	}
	signerB, err := helpers.NewEd25519SignerFromSeed("agent.test", seed)
	if err != nil {
		t.Fatal(err)
	}
	body := []byte(`{"hello":"world"}`)
...
func TestAppendSignature_TwiceAppends(t *testing.T) {
	...
	if err := helpers.AppendSignature(ctx, req, body, s2, mk(1)); err != nil {
		t.Fatalf("append sig2: %v", err)
	}
	if err := helpers.AppendSignature(ctx, req, body, s3, mk(2)); err != nil {
...
func TestAppendSignature_PreservesExistingHeaders(t *testing.T) {
	...
	resolver := helpers.NewStaticKeyResolver(map[string]ed25519.PublicKey{"first.test": p1})
	v, err := helpers.VerifyRequestResolved(ctx, req, body, resolver, helpers.VerifyOptions{Now: now})
	if err != nil {
