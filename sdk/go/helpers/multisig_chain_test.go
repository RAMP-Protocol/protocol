...
func TestAppendSignature_twoHopChainVerifies(t *testing.T) {
	...
	agentSigner, err := helpers.NewEd25519Signer("agent.k1", agentPriv)
	if err != nil {
		t.Fatal(err)
	}
	// A broker relay key uses the broker.* keyID convention.
	brokerSigner, err := helpers.NewEd25519Signer(helpers.BrokerKeyIDPrefix+"inst.v1", brokerPriv)
	if err != nil {
		t.Fatal(err)
	}

	now := time.Unix(1_700_000_000, 0)
	opts := helpers.SignOptions{Created: now.Unix(), Expires: now.Add(time.Hour).Unix()}
	...
	resolver := helpers.NewStaticKeyResolver(map[string]ed25519.PublicKey{
		"agent.k1":                            agentPub,
		helpers.BrokerKeyIDPrefix + "inst.v1": brokerPub,
	})
	vopts := helpers.VerifyOptions{Now: now.Add(time.Minute)}

	verified, err := helpers.VerifyMultisigRequestResolved(ctx, req, body, resolver, vopts)
	if err != nil {
...
func TestAppendSignature_tamperedPredecessorRejected(t *testing.T) {
	...
	agentPub, agentPriv, _ := ed25519.GenerateKey(nil)
	brokerPub, brokerPriv, _ := ed25519.GenerateKey(nil)
	agentSigner, _ := helpers.NewEd25519Signer("agent.k1", agentPriv)
	brokerSigner, _ := helpers.NewEd25519Signer(helpers.BrokerKeyIDPrefix+"inst.v1", brokerPriv)

	now := time.Unix(1_700_000_000, 0)
	opts := helpers.SignOptions{Created: now.Unix(), Expires: now.Add(time.Hour).Unix()}
	req, body := multisigReq(t)
	_ = helpers.SignRequest(ctx, req, body, agentSigner, opts)
	_ = helpers.AppendSignature(ctx, req, body, brokerSigner, opts)
	...
	otherSigner, _ := helpers.NewEd25519Signer("agent.k1", otherPriv)
	...
	if _, err := helpers.VerifyMultisigRequestResolved(ctx, req, body, resolver, vopts); err == nil {
