...
var ErrEmptyMoney = errors.New("ramphelpers: empty money string (field is unset)")
...
func ParseMoney(s string) (decimal.Decimal, error) {
	...
	if !moneyWire.MatchString(s) {
		return decimal.Decimal{}, fmt.Errorf("ramphelpers: %q is not a canonical money string (want %s)", s, moneyWire.String())
	}
	d, err := decimal.NewFromString(s)
	if err != nil {
		return decimal.Decimal{}, fmt.Errorf("ramphelpers: parse money %q: %w", s, err)
	}
	return d, nil
}
...
func FormatMoney(d decimal.Decimal) (string, error) {
	if d.IsNegative() {
		return "", fmt.Errorf("ramphelpers: negative money %s is not representable on the wire", d.String())
	}
