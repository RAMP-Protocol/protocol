...
var ErrOfferSignatureInvalid = errors.New("ramphelpers: offer signature invalid")
...
func VerifyOffer(offer *rampv1.Offer, signatureHex string, pub ed25519.PublicKey) error {
	...
	if err != nil {
		return fmt.Errorf("ramphelpers: decode offer signature: %w", err)
	}
	payload, err := canonicalOfferPayload(offer)
	if err != nil {
		return err
...
func canonicalOfferPayload(offer *rampv1.Offer) ([]byte, error) {
	...
	if !ok {
		return nil, errors.New("ramphelpers: offer clone type mismatch")
	}
	clone.Signature = ""
	clone.SignatureAlgorithm = ""
	data, err := proto.MarshalOptions{Deterministic: true}.Marshal(clone)
	if err != nil {
		return nil, fmt.Errorf("ramphelpers: marshal offer: %w", err)
	}
	return data, nil
}
