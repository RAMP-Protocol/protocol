...
func TestThumbprintSharedVectors(t *testing.T) {
	...
	for _, v := range vf.Vectors {
		t.Run(v.Thumbprint, func(t *testing.T) {
			...
			got, err := ramphelpers.Thumbprint(ed25519.PublicKey(pub))
			if err != nil {
...
func TestThumbprintBytesMatchesString(t *testing.T) {
	...
	sum, err := ramphelpers.ThumbprintBytes(ed25519.PublicKey(pub))
	if err != nil {
...
func TestThumbprintRejectsBadLength(t *testing.T) {
	...
	for _, n := range []int{0, 16, 31, 33, 64} {
		if _, err := ramphelpers.Thumbprint(make(ed25519.PublicKey, n)); err == nil {
...
func TestThumbprintRealKey(t *testing.T) {
	...
	tp, err := ramphelpers.Thumbprint(pub)
	if err != nil {
		t.Fatalf("Thumbprint: %v", err)
	}
