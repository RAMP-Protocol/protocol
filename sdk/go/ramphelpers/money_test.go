...
func TestParseMoney_valid(t *testing.T) {
	...
	for in, wantCanonical := range cases {
		d, err := ramphelpers.ParseMoney(in)
		if err != nil {
			t.Errorf("ParseMoney(%q) error: %v", in, err)
			continue
		}
		got, err := ramphelpers.FormatMoney(d)
		if err != nil {
...
func TestParseMoney_empty(t *testing.T) {
	if _, err := ramphelpers.ParseMoney(""); !errors.Is(err, ramphelpers.ErrEmptyMoney) {
...
func TestParseMoney_rejectsNonCanonical(t *testing.T) {
	...
	for _, bad := range []string{"-1", "1e3", "1E3", ".5", "1.", "+1", "0x10", "1,000", " 1", "1 ", "abc"} {
		if _, err := ramphelpers.ParseMoney(bad); err == nil {
...
func TestFormatMoney_canonical(t *testing.T) {
	...
	for _, c := range cases {
		got, err := ramphelpers.FormatMoney(c.in)
		if err != nil {
		...
		if _, err := ramphelpers.ParseMoney(got); err != nil {
...
func TestFormatMoney_rejectsNegative(t *testing.T) {
	if _, err := ramphelpers.FormatMoney(decimal.RequireFromString("-0.01")); err == nil {
...
func TestMoney_exactArithmetic(t *testing.T) {
	...
	a, _ := ramphelpers.ParseMoney("0.1")
	b, _ := ramphelpers.ParseMoney("0.2")
	got, err := ramphelpers.FormatMoney(a.Add(b))
	if err != nil {
		t.Fatal(err)
	}
	if got != "0.3" {
...
func TestCanonicalizeMoney(t *testing.T) {
	got, err := ramphelpers.CanonicalizeMoney("0.050")
	if err != nil {
		t.Fatal(err)
	}
	if got != "0.05" {
