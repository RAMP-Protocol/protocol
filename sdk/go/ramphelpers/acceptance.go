...
// idempotency key).
var ErrAcceptanceSignatureInvalid = errors.New("ramphelpers: offer-acceptance signature invalid")
...
func canonicalAcceptancePayload(offer *rampv1.Offer, requester *rampv1.Requester, idempotencyKey string) ([]byte, error) {
	if offer == nil {
		return nil, errors.New("ramphelpers: offer is nil")
	}
	if requester == nil {
		return nil, errors.New("ramphelpers: requester is nil")
	}
	if offer.GetSignature() == "" {
		return nil, errors.New("ramphelpers: cannot accept an unsigned offer (empty offer signature)")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("ramphelpers: marshal acceptance payload: %w", err)
	}
	return data, nil
}
...
func SignOfferAcceptance(priv ed25519.PrivateKey, offer *rampv1.Offer, requester *rampv1.Requester, idempotencyKey string) (string, error) {
	if len(priv) != ed25519.PrivateKeySize {
		return "", fmt.Errorf("ramphelpers: ed25519 private key must be %d bytes, got %d", ed25519.PrivateKeySize, len(priv))
	}
...
func VerifyOfferAcceptance(offer *rampv1.Offer, requester *rampv1.Requester, idempotencyKey, signatureHex string, pub ed25519.PublicKey) error {
	if len(pub) != ed25519.PublicKeySize {
		return fmt.Errorf("ramphelpers: ed25519 public key must be %d bytes, got %d", ed25519.PublicKeySize, len(pub))
	}
	sig, err := hex.DecodeString(signatureHex)
	if err != nil {
		return fmt.Errorf("ramphelpers: decode acceptance signature: %w", err)
	}
	payload, err := canonicalAcceptancePayload(offer, requester, idempotencyKey)
	if err != nil {
		return err
