...
func TestNewIdempotencyKey_formatAndUniqueness(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		k, err := ramphelpers.NewIdempotencyKey()
		if err != nil {
			t.Fatal(err)
		}
		if err := ramphelpers.ValidateIdempotencyKey(k); err != nil {
...
func TestValidateIdempotencyKey_empty(t *testing.T) {
	if err := ramphelpers.ValidateIdempotencyKey(""); !errors.Is(err, ramphelpers.ErrEmptyIdempotencyKey) {
