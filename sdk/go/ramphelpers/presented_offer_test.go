...
func TestVerifyPresentedOffer_valid(t *testing.T) {
	...
	if err := ramphelpers.VerifyPresentedOffer(offer, pub, now); err != nil {
...
func TestVerifyPresentedOffer_tamperedPrice(t *testing.T) {
	...
	now := time.Unix(tsBeforeExpiry, 0)
	if err := ramphelpers.VerifyPresentedOffer(offer, pub, now); !errors.Is(err, ramphelpers.ErrOfferSignatureInvalid) {
...
func TestVerifyPresentedOffer_wrongKey(t *testing.T) {
	...
	now := time.Unix(tsBeforeExpiry, 0)
	if err := ramphelpers.VerifyPresentedOffer(offer, other, now); !errors.Is(err, ramphelpers.ErrOfferSignatureInvalid) {
...
func TestVerifyPresentedOffer_expired(t *testing.T) {
	...
	if err := ramphelpers.VerifyPresentedOffer(offer, pub, now); !errors.Is(err, ramphelpers.ErrOfferExpired) {
