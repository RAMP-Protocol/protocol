...
func fixedSigner(t *testing.T, keyID string, b byte) (ramphelpers.Signer, ed25519.PublicKey) {
	t.Helper()
...
func goldenSignedRequest(t *testing.T, body []byte, signer ramphelpers.Signer, now time.Time) *http.Request {
	t.Helper()
	...
	opts := ramphelpers.SignOptions{Created: now.Unix(), Expires: now.Add(30 * time.Second).Unix()}
	if err := ramphelpers.SignRequest(context.Background(), req, body, signer, opts); err != nil {
...
func TestChain_GoldenSignatureBase(t *testing.T) {
	...
	brokerSigner, _ := fixedSigner(t, ramphelpers.BrokerKeyIDPrefix+"relay.a", 0x02)
...
func chainEnv(t *testing.T, now time.Time) (*http.Request, []byte, ramphelpers.KeyResolver) {
	t.Helper()
	ctx := context.Background()
	agentSigner, agentPub := fixedSigner(t, "agent-demo.v1", 0x05)
	brokerSigner, brokerPub := fixedSigner(t, ramphelpers.BrokerKeyIDPrefix+"relay.a", 0x06)
	body := []byte(`{"q":"chain"}`)
	req := goldenSignedRequest(t, body, agentSigner, now)
	opts := ramphelpers.SignOptions{Created: now.Unix(), Expires: now.Add(30 * time.Second).Unix()}
	if err := ramphelpers.AppendSignature(ctx, req, body, brokerSigner, opts); err != nil {
	...
	resolver := ramphelpers.NewStaticKeyResolver(map[string]ed25519.PublicKey{
		...
		ramphelpers.BrokerKeyIDPrefix + "relay.a": brokerPub,
	})
...
func TestChain_ValidChainVerifies(t *testing.T) {
	...
	verified, err := ramphelpers.VerifyMultisigRequestResolved(context.Background(), req, body, resolver, ramphelpers.VerifyOptions{Now: now})
	if err != nil {
...
func TestChain_StrippedMiddleHop(t *testing.T) {
	...
	s2, p2 := fixedSigner(t, ramphelpers.BrokerKeyIDPrefix+"relay.a", 0x08)
	s3, p3 := fixedSigner(t, ramphelpers.BrokerKeyIDPrefix+"relay.b", 0x09)
	body := []byte(`{"q":"three"}`)
	req := goldenSignedRequest(t, body, s1, now)
	opts := ramphelpers.SignOptions{Created: now.Unix(), Expires: now.Add(30 * time.Second).Unix()}
	if err := ramphelpers.AppendSignature(ctx, req, body, s2, opts); err != nil {
		t.Fatalf("append sig2: %v", err)
	}
	if err := ramphelpers.AppendSignature(ctx, req, body, s3, opts); err != nil {
		t.Fatalf("append sig3: %v", err)
	}
	resolver := ramphelpers.NewStaticKeyResolver(map[string]ed25519.PublicKey{
		"agent-demo.v1": p1,
		ramphelpers.BrokerKeyIDPrefix + "relay.a": p2,
		ramphelpers.BrokerKeyIDPrefix + "relay.b": p3,
	})
	...
	_, err := ramphelpers.VerifyMultisigRequestResolved(ctx, req, body, resolver, ramphelpers.VerifyOptions{Now: now})
	if !errors.Is(err, ramphelpers.ErrBrokenSignatureChain) {
...
func TestChain_Reordered(t *testing.T) {
	...
	_, err := ramphelpers.VerifyMultisigRequestResolved(context.Background(), req, body, resolver, ramphelpers.VerifyOptions{Now: now})
	if !errors.Is(err, ramphelpers.ErrBrokenSignatureChain) {
		t.Fatalf("want ErrBrokenSignatureChain, got %v", err)
	}
...
func TestChain_MissingLink(t *testing.T) {
	...
	s2, p2 := fixedSigner(t, ramphelpers.BrokerKeyIDPrefix+"relay.a", 0x0b)
	body := []byte(`{"q":"nolink"}`)
...
func TestChain_SubstitutedPredecessorRejected(t *testing.T) {
	...
	brokerSigner, brokerPub := fixedSigner(t, ramphelpers.BrokerKeyIDPrefix+"relay.a", 0x0d)
	body := []byte(`{"q":"subst"}`)
	...
	_, err := ramphelpers.VerifyMultisigRequestResolved(ctx, req, body, resolver, ramphelpers.VerifyOptions{Now: now.Add(time.Second)})
...
func TestChain_TooManyHops(t *testing.T) {
	now := time.Unix(1700000000, 0)
	req, body, resolver := chainEnv(t, now)
	_, err := ramphelpers.VerifyMultisigRequestResolved(context.Background(), req, body, resolver,
		ramphelpers.VerifyOptions{Now: now, MaxSignatures: 1})
	if !errors.Is(err, ramphelpers.ErrTooManyHops) {
...
func TestChain_WithinHopBudget(t *testing.T) {
	now := time.Unix(1700000000, 0)
	req, body, resolver := chainEnv(t, now)
	if _, err := ramphelpers.VerifyMultisigRequestResolved(context.Background(), req, body, resolver,
		ramphelpers.VerifyOptions{Now: now, MaxSignatures: 2}); err != nil {
