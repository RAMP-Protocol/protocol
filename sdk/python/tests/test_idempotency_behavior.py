...
new_idempotency_key carries entropy → NOT vector-gated. The Go oracle mints 16
crypto-random bytes → base64.RawURLEncoding (22 chars, no padding). The Python
face ``new_idempotency_key()`` MUST match that SHAPE: a 22-char base64url string,
...
from ramp_sdk.idempotency import new_idempotency_key  # type: ignore[import-not-found]
...
def test_mints_distinct_keys_across_n_calls() -> None:
    n = 512
    seen = {new_idempotency_key() for _ in range(n)}
    assert len(seen) == n
